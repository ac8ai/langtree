...
class PromptScope(Scope):
    ...
    def __init__(self, node_tag: Optional[str] = None):
        """
...
class ValueScope(Scope):
    ...
    def __init__(self, root_tag: Optional[str] = None):
        """
...
def get_scope(scope_name: str, **kwargs) -> Scope:
    ...
    if scope is None:
        raise ValueError(f"Unknown scope: {scope_name}. Available: {list(_SCOPES.keys())}")
    return scope
...
class ContextResolver(ABC):
    ...
    @abstractmethod
    def resolve(self, path: str, scope: Optional['StructureTreeNode'] = None, 
                target_node: Optional['StructureTreeNode'] = None) -> str:
...
class ContextResolverFactory:
    ...
    def get_resolver(cls, context_type: str) -> ContextResolver:
        ...
        if context_type not in cls._resolvers:
            raise ValueError(f"Unknown context type: {context_type}. Available: {list(cls._resolvers.keys())}")
