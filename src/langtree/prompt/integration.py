...
class LangTreeChainBuilder:
    ...
    def build_execution_chain(
        self, llm_name: str = "reasoning", **chain_kwargs
    ) -> "Runnable":
        """
        ...
        if assembly_validation_errors:
            raise ValueError(
                f"Assembly validation failed: {assembly_validation_errors}"
            )
    ...
    def _resolve_pending_targets(self) -> None:
        ...
        pending_targets = dict(
            self.run_structure._pending_target_registry.pending_targets
        )
        ...
        for target_path in list(pending_targets.keys()):
            ...
            if target_node is not None:
                ...
                self.run_structure._pending_target_registry.resolve_target(
        ...
        for step in base_plan["chain_steps"]:
            ...
            if node and hasattr(node, "extracted_commands"):
                for command in node.extracted_commands:
                    if (
                        hasattr(command, "destination_path")
                        and command.destination_path
                    ):
                        # Find the target node for this command
                        target_node = self.run_structure.get_node(
                            command.destination_path
                        )
                        if not target_node:
                            # Try alternative paths
                            alternatives = self._generate_alternative_target_paths(
                                command.destination_path
                            )
    ...
    def _topological_sort(
        self, steps: list[dict[str, Any]], dependencies: dict[str, set]
    ) -> list[dict[str, Any]]:
        """
    ...
    def _build_step_chain(
        self, step: dict[str, Any], llm_name: str, **chain_kwargs
    ) -> "Runnable":
        """
    ...
    def _compose_dependency_chain(
        self, chain_steps: dict[str, "Runnable"], execution_plan: dict[str, Any]
    ) -> "Runnable":
        """
        ...
        for key, step in chain_steps.items():
            ...
            # Check if it's already a Runnable or callable
                ...
                converted_steps[key] = (
                    RunnableLambda(step) if not hasattr(step, "invoke") else step
                )
            else:
    ...
    def _validate_dependencies(
        self, chain_steps: dict[str, "Runnable"], chain_steps_list: list[dict[str, Any]]
    ) -> None:
        """
        ...
        for step in chain_steps_list:
            ...
            if not isinstance(deps, list):
                raise ValueError(
                    f"Invalid dependencies format for node '{node_tag}': dependencies must be a list"
                )
        ...
        for node_tag, deps in dependencies.items():
            for dep in deps:
                if dep not in all_nodes:
                    raise ValueError(
                        f"Missing dependency: Node '{node_tag}' depends on nonexistent node '{dep}'"
                    )
        ...
        def has_cycle(node, path):
            if node in rec_stack:
                ...
                if len(cycle_path) == 2 and cycle_path[0] == cycle_path[1]:
                    raise ValueError(
                        f"Self-dependency detected: Node '{node}' depends on itself"
                    )
                else:
                    raise ValueError(
    ...
    def _identify_parallel_groups(
        self, dependency_order: list[str], execution_plan: dict[str, Any]
    ) -> list[list[str]]:
        """
        ...
        def traverse_nodes(node, node_tag):
            ...
            if hasattr(node, "extracted_commands"):
                for command in node.extracted_commands:
                    ...
                    if hasattr(command, "variable_mappings") and hasattr(
                        command, "inclusion_path"
                    ):
                        # Convert node tag to full path if needed
                        full_node_tag = (
                            node_tag if "." in node_tag else f"task.{node_tag}"
                        )
                        try:
                            self._validate_destination_field_nesting(
                                command, full_node_tag
                            )
                        except Exception as e:
                            validation_errors.append(
                                f"LHS-RHS nesting validation failed for {node_tag}: {e}"
                            )
    ...
    def _validate_destination_field_nesting(
        self, command, source_node_tag: str
    ) -> None:
        """
        ...
        iteration_levels = self.run_structure._count_iterable_levels_in_path(
            ...
            if (
                ...
                if target_scope and target_scope.get_name() == "value":
                    ...
                    if not field_exists:
                        ...
                        destination_nesting_levels.append(
                            0
                        )  # Destination fields are typically simple
        ...
        if destination_nesting_levels and iteration_levels > 0:
            if not any(
                level == iteration_levels for level in destination_nesting_levels
            ):
                raise ValueError(
...
class PromptAssembler:
    ...
    def assemble_prompt(self, node: "StructureTreeNode") -> dict[str, str]:
        ...
        prompt_sections = {
            ...
            "task": self._extract_task_section(
                system_prompt, processed_field_descriptions
            ),
    ...
    def _extract_task_section(
        self, system_prompt: str, field_descriptions: dict[str, str]
    ) -> str:
        """
    ...
    def _extract_output_section(self, field_descriptions: dict[str, str]) -> str | None:
        """
...
class ContextPropagator:
    ...
    def wrap_with_context_propagation(
        self, base_chain: "Runnable", node_tag: str, node: "StructureTreeNode"
    ) -> "Runnable":
        """
        ...
        def context_wrapper(input_data: dict[str, Any]) -> Any:
            ...
            if "prompt_template" in input_data:
                resolved_template = resolve_runtime_variables(
                    input_data["prompt_template"], self.run_structure, node
                )
