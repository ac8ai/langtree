...
def validate_tree(run_structure: "RunStructure") -> dict[str, list[str]]:
    ...
    for (
        target_path,
        pending_list,
    ) in run_structure._pending_target_registry.pending_targets.items():
...
def _detect_circular_dependencies(run_structure: "RunStructure", result: dict) -> None:
    ...
    for node_name, node in run_structure._root_nodes.items():
        if node_name not in visited:
            cycle = _find_target_cycle(
                run_structure, node, visited, current_path.copy()
            )
    ...
    if hasattr(node, "extracted_commands"):
        for command in node.extracted_commands:
            if command.destination_path:
                ...
                if target_node:
                    cycle = _find_target_cycle(
                        run_structure, target_node, visited, current_path
                    )
                    if cycle:
...
def _detect_unsatisfied_variables(run_structure: "RunStructure", result: dict) -> None:
    ...
    Params:
        ...
        result: Dictionary to accumulate validation results
    """
    unsatisfied_vars = run_structure._variable_registry.get_truly_unsatisfied_variables(
        run_structure
    )
...
def _detect_invalid_scope_references(
    run_structure: "RunStructure", result: dict
) -> None:
...
def _check_invalid_scopes_recursive(
    node: "StructureTreeNode", result: dict, valid_scopes: set
) -> None:
        ...
        for command in node.extracted_commands:
            ...
            for mapping in command.variable_mappings:
                ...
                if len(target_parts) > 1:
                    ...
                    if (
                        potential_scope not in valid_scopes
                        and potential_scope != "current_node"
                    ):
...
def _check_impossible_mappings_recursive(
    run_structure: "RunStructure", node: "StructureTreeNode", result: dict
) -> None:
            ...
            if (
                ...
                if hasattr(command, "inclusion_path") and command.inclusion_path:
                    try:
                        ...
                        resolved_value = _resolve_in_current_node_context(
                            run_structure, command.inclusion_path, node.name
                        )
