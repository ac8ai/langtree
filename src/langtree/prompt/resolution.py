...
from langtree.prompt.registry import _validate_path_and_node_tag, _get_node_instance
...
if TYPE_CHECKING:
    ...
    from langtree.prompt.structure import (
        RunStructure, StructureTreeNode, PromptValue, ResolutionResult
    )
...
def _resolve_node_contexts_recursive(run_structure: 'RunStructure', node: 'StructureTreeNode', results: dict) -> None:
    """
    ...
    if hasattr(node, 'extracted_commands'):
        for command in node.extracted_commands:
            try:
                ...
                target_node = run_structure.get_node(command.destination_path) if command.destination_path else None
...
def _resolve_command_context(run_structure: 'RunStructure', command: 'ParsedCommand', 
                           source_node_tag: str, target_node: 'StructureTreeNode | None') -> None:
    """
...
def _resolve_inclusion_context(run_structure: 'RunStructure', resolved_inclusion: Any, source_node_tag: str):
    """
    ...
    try:
        ...
        if inclusion_scope:
            ...
            inclusion_value = _resolve_in_current_node_context(run_structure, inclusion_path, source_node_tag)
        
        # Validate that the inclusion target is iterable
        if not hasattr(inclusion_value, '__iter__') or isinstance(inclusion_value, str):
            raise ValueError(f"Inclusion path '{inclusion_path}' resolves to non-iterable value: {type(inclusion_value)}")
        ...
        raise ValueError(f"Failed to resolve inclusion path '{inclusion_path}' in node '{source_node_tag}': {e}")


def _resolve_destination_context(run_structure: 'RunStructure', resolved_destination: Any, 
                                destination_path: str, target_node: 'StructureTreeNode | None'):
    ...
    try:
        if resolved_destination and resolved_destination.scope:
            ...
            if scope_name == 'task':
                ...
                if not target_node.field_type:
                    raise ValueError(f"Target node has no field type for {scope_name} scope validation")
        ...
        return {
            ...
            'target_type': target_node.field_type.__name__ if target_node.field_type else 'unknown',
            'scope': resolved_destination.scope.get_name() if resolved_destination and resolved_destination.scope else 'direct',
            'requires_resolution': False
        }
...
def _resolve_variable_mapping_context(run_structure: 'RunStructure', resolved_target: Any, 
                                    resolved_source: Any, source_node_tag: str, 
    ...
    if not resolved_target or not resolved_source:
        raise ValueError("Both target and source must be resolved for mapping validation")
    
    mapping_result = {
        'target_path': resolved_target.path,
        'source_path': resolved_source.path,
        'source_node': source_node_tag,
        'target_scope': resolved_target.scope.get_name() if resolved_target.scope else 'current_node',
        'source_scope': resolved_source.scope.get_name() if resolved_source.scope else 'current_node',
        'validations': {}
    }
    ...
    try:
        if resolved_source.path == "*":
            ...
            if resolved_source.scope:
                ...
                # Default to current node context
                try:
                    source_value = _resolve_in_current_node_context(run_structure, resolved_source.path, source_node_tag)
                    mapping_result['validations']['source'] = {
                        'status': 'valid',
                        'type': type(source_value).__name__,
        ...
        if target_node is None:
            ...
            if resolved_target.scope:
                ...
                if scope_name == 'outputs':
                    ...
                    _track_outputs_collection(run_structure, resolved_target.path,
                                            resolved_source.path, source_node_tag, target_node)
                    mapping_result['validations']['target'] = {
                        'status': 'valid',
                        'scope': 'outputs',
                    ...
                    try:
                        _resolve_in_target_node_context(run_structure, resolved_target.path, target_node)
                        mapping_result['validations']['target'] = {
                            'status': 'valid',
                            'scope': 'value',
...
def _resolve_in_current_node_context(run_structure: 'RunStructure', path: str, node_tag: str) -> 'PromptValue':
    ...
    try:
        return _navigate_object_path(node_instance, path, "current node context", node_tag)
...
def _is_missing_field_acceptable(path: str, node_instance: object) -> bool:
    ...
    try:
        ...
        if first_component in model_fields:
            ...
            if hasattr(field_info, 'default') or not getattr(field_info, 'is_required', True):
                return True
...
def _resolve_scope_segment_context(run_structure: 'RunStructure', path: str, node_tag: str) -> 'ResolutionResult':
...
def _resolve_in_value_context(run_structure: 'RunStructure', path: str, node_tag: str) -> 'PromptValue':
...
def _track_outputs_collection(run_structure: 'RunStructure', target_path: str,
                             source_path: str, source_node_tag: str,
                             target_node: 'StructureTreeNode | None') -> None:
...
def _resolve_in_outputs_context(run_structure: 'RunStructure', path: str, node_tag: str) -> 'PromptValue':
...
def _resolve_in_global_tree_context(run_structure: 'RunStructure', path: str) -> 'ResolutionResult':
    ...
    if path_parts[0] != 'task' or len(path_parts) < 2:
        raise ValueError(f"Path '{path}' must start with 'task.' and have at least one node component")
    ...
    if len(node_path_parts) > 1:
        ...
        if target_node:
            ...
            return _navigate_object_path(node_instance, field_name, "global tree context", full_node_path)
    ...
    if original_path in pending_targets:
        ...
        raise KeyError(f"Forward reference to '{path}' not yet resolved. "
...
def _resolve_in_target_node_context(run_structure: 'RunStructure', path: str, 
                                   target_node: 'StructureTreeNode | None') -> bool:
    ...
    # Target presence
    if target_node is None:
        raise ValueError("Target node cannot be None - pending targets not supported in structure validation")
    
    # Schema presence
    if not target_node.field_type:
        raise AttributeError(f"Target node '{target_node.name}' has no field type defined")
    ...
    try:
        ...
        _navigate_object_path(target_instance, path, "target node context", target_node.name)
        return True  # Path exists and is valid
    except (KeyError, AttributeError, ValueError) as e:
        # Re-raise with more specific context for target validation
        raise KeyError(f"Path '{path}' not found in target node '{target_node.name}' structure: {e}")


def _resolve_in_target_outputs_context(run_structure: 'RunStructure', path: str, 
                                      target_node: 'StructureTreeNode | None'):
...
def _resolve_in_target_prompt_context(run_structure: 'RunStructure', path: str, 
                                     target_node: 'StructureTreeNode | None'):
...
def _resolve_in_current_prompt_context(run_structure: 'RunStructure', path: str, node_tag: str):
...
def _navigate_object_path(obj: object, path: str, context_name: str, node_tag: str) -> 'PromptValue':
    ...
    for i, part in enumerate(path_parts):
        if not part:  # Handle empty parts from malformed paths
            raise ValueError(f"Malformed path '{path}': empty component at position {i}")
        ...
        try:
            if hasattr(current_obj, part):
                ...
                if i == len(path_parts) - 1:  # Last part of path
                    ...
                    if current_obj and hasattr(current_obj[0], part) if current_obj else False:
                        # Return a list of the attribute values from all items
                        return [getattr(item, part) for item in current_obj if hasattr(item, part)]
                    else:
                    ...
                    raise KeyError(f"Cannot navigate deeper into list at path component '{part}'")
            else:
                ...
                raise KeyError(f"Path component '{part}' not found in {context_name} for {type(current_obj).__name__}")
        except (AttributeError, KeyError, TypeError) as e:
            remaining_path = '.'.join(path_parts[i:])
            raise KeyError(f"Cannot resolve path '{remaining_path}' in {context_name} for node '{node_tag}': {e}")
    ...
    def resolve_variable(match):
        ...
        try:
            ...
            if '{' in variable_path or '}' in variable_path:
                raise RuntimeVariableError(f"Malformed variable syntax. Use single braces {{variable}} for runtime variables, not {{{{{variable_path}}}}}.")

            # Runtime variables must be single tokens (no dots, no double underscores)
            if '.' in variable_path:
                raise RuntimeVariableError(f"Runtime variable '{{{variable_path}}}' cannot contain dots. Use single token names only.")

            if '__' in variable_path:
                raise RuntimeVariableError(f"Runtime variable '{{{variable_path}}}' cannot contain double underscores. Double underscores are reserved for system use.")
            ...
            if validate and current_node and hasattr(current_node, 'field_type') and current_node.field_type:
                try:
                    ...
                    if run_structure and hasattr(run_structure, '_assembly_variable_registry'):
                        try:
                            assembly_vars_list = run_structure._assembly_variable_registry.get_variables_for_node(current_node.name)
                            assembly_variables = [var.name for var in assembly_vars_list]
                    ...
                    if not is_field_variable:
                        ...
                        if assembly_variables:
                            assembly_info = f"Available assembly variables: {assembly_variables}. "
                    ...
                    import warnings
                    warnings.warn(f"Unexpected validation error in runtime variable resolution: {validation_error}")
...
def _resolve_node_field_variable(
    ...
    try:
        ...
        if current_node.parent:
            try:
                parent_tag = current_node.parent.name
                result = _resolve_in_current_node_context(run_structure, field_name, parent_tag)
                return str(result) if result is not None else ""
            except Exception:
                pass
    
    from langtree.prompt.exceptions import RuntimeVariableError
    raise RuntimeVariableError(f"Failed to resolve runtime variable '{{{field_name}}}': field not found in current or parent context")
