...
from langtree.prompt.exceptions import TemplateVariableSpacingError, TemplateVariableConflictError, TemplateVariableNameError

if TYPE_CHECKING:
    from langtree.prompt.structure import StructureTreeNode, RunStructure
...
TEMPLATE_VARIABLES_PATTERN = re.compile(r'(?<!\{)\{(?:PROMPT_SUBTREE|COLLECTED_CONTEXT)\}(?!\})')
...
def validate_template_variable_names(content: str) -> list[str]:
    ...
    for match in nested_matches:
        ...
        errors.append(f"Malformed template variable syntax with nested braces '{match.group()}' at line {line_number}. Template variables cannot be nested.")
    ...
    for match in matches:
        ...
        if var_name not in ('PROMPT_SUBTREE', 'COLLECTED_CONTEXT'):
            line_number = content[:match.start()].count('\n') + 1
            errors.append(f"Unknown template variable '{{{var_name}}}' at line {line_number}. Only PROMPT_SUBTREE and COLLECTED_CONTEXT are supported.")
    ...
    for match in runtime_matches:
        ...
        if '__' in var_name:
            line_number = content[:match.start()].count('\n') + 1
            errors.append(f"Runtime variable '{{{var_name}}}' at line {line_number} contains double underscore '__' which is reserved for system use.")
...
def validate_template_variable_spacing(content: str) -> list[str]:
    ...
    for i in range(len(template_vars) - 1):
        ...
        if current_match.end() == next_match.start():
            ...
            errors.append(f"Template variables {current_match.group()} at line {current_line} and {next_match.group()} at line {next_line} must have empty lines between them")
    ...
    for match in template_vars:
        ...
        if not before_valid or not after_valid:
            errors.append(f"Template variable {var_name} at line {line_number} requires empty lines before and after")
...
def validate_template_variable_conflicts(content: str, assembly_variables: set[str]) -> list[str]:
    """
    ...
    for var_name in assembly_variables:
        if var_name in template_var_names:
            errors.append(f"Assembly Variable '{var_name}' conflicts with template variable {{{var_name}}}")
...
def get_assembly_variables_for_node_with_structure(node: 'StructureTreeNode', run_structure: 'RunStructure') -> set[str]:
    """
...
def process_template_variables(content: str, node: Optional['StructureTreeNode'] = None) -> str:
    """
    ...
    if name_errors:
        raise TemplateVariableNameError(f"Template variable name errors: {'; '.join(name_errors)}")

    # Validate spacing on clean content
    spacing_errors = validate_template_variable_spacing(clean_content)
    if spacing_errors:
        raise TemplateVariableSpacingError(f"Template variable spacing errors: {'; '.join(spacing_errors)}")
    ...
    if node:
        ...
        conflict_errors = validate_template_variable_conflicts(clean_content, assembly_vars)
        if conflict_errors:
            raise TemplateVariableConflictError(f"Template variable conflicts: {'; '.join(conflict_errors)}")
...
def resolve_prompt_subtree(node: 'StructureTreeNode', base_heading_level: int = 1) -> str:
    """
...
def resolve_collected_context(node: 'StructureTreeNode', context_data: Optional[str] = None) -> str:
    """
...
def resolve_template_variables_in_content(content: str, node: 'StructureTreeNode') -> str:
    """
