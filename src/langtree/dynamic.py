...
def resolve_type(field: NestedStrDict, custom_types: dict[str, type] = {}) -> UnionType | type | None:
    ...
    if 'type' in field:
        ...
        if not isinstance(raw_type, str):
            raise ValueError(f"Invalid 'type' value; expected string, got: {type(raw_type)}")
        ...
        for frag in any_of_list[1:]:
            ...
            if frag_type is None:
                raise ValueError(f"Could not resolve union fragment type '{frag_type_name}'")
            if t is None:
                t = frag_type
            else:
        ...
        raise ValueError("Field does not have a `type` or `anyOf` defined. Field json:\n" + str(field))
    return t
...
def describe_datafield_type(field: NestedStrDict) -> str:
    ...
    if not isinstance(type_descr, str):
        raise ValueError(f"Field type is not a string: {type_descr}. Field json:\n{field}")
    return type_descr
...
def schema_to_model(
    ...
    for type_name, type_schema in defs.items():  # type: ignore[assignment]
        if (type_name in custom_types and not replace_existing) or type_name in new_types:
            continue
        if not isinstance(type_schema, dict):
            raise ValueError(f"Definition for {type_name} must be a dict schema fragment")
    ...
    for field_name, type_schema in properties.items():
        ...
        fields[field_name] = (resolved or (str | None), Field(description=description_val))  # type: ignore
