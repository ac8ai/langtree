...
class VariableMapping:
    ...
    resolved_target: Optional[ResolvedPath] = None
    resolved_source: Optional[ResolvedPath] = None
...
class ExecutionCommand:
    ...
    command_name: str
    arguments: List[str | int | float | bool]  # Positional arguments (variables or literals)
    named_arguments: Optional[dict[str, str | int | float | bool]] = None  # Named arguments (key=value)
    comment: Optional[str] = None
    ...
    def __post_init__(self):
        ...
        if self.command_name not in self.VALID_COMMANDS:
            raise CommandParseError(f"Unknown command: {self.command_name}. Valid commands are: {', '.join(sorted(self.VALID_COMMANDS))}")
    ...
    def _validate_resample_args(self):
        ...
        if total_args != 1:
            raise CommandParseError(f"resample command requires exactly 1 argument (n_times), got {total_args}")
        
        # Argument can be int literal or variable name (string) - NOT boolean
        arg = self.arguments[0]
        if isinstance(arg, bool):
            # Boolean values are not valid for resample
            raise CommandParseError(f"resample argument must be int or variable name, got {type(arg).__name__}: {arg}")
        elif isinstance(arg, str):
            # Variable name - validate format
            if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', arg):
                raise CommandParseError(f"Invalid variable name in resample argument: {arg}")
    ...
    def _validate_llm_args(self):
        ...
        if not (1 <= total_args <= 2):
            raise CommandParseError(f"llm command requires 1-2 arguments (model_key, [override]), got {total_args}")
        
        # First argument: model_key (string literal or variable name) - always positional
        if len(self.arguments) == 0:
            raise CommandParseError("llm command requires model_key as first positional argument")
        
        model_key = self.arguments[0]
        if not isinstance(model_key, str):
            raise CommandParseError(f"llm model_key must be string, got {type(model_key).__name__}: {model_key}")
        ...
        if total_args == 2:
            if len(self.arguments) == 2:
                ...
                unknown_args = list(self.named_arguments.keys()) if self.named_arguments else []
                raise CommandParseError(f"llm command only supports 'override' named argument, got: {', '.join(unknown_args)}")
    ...
    def _validate_override_value(self, override):
        ...
        if isinstance(override, str):
            ...
            if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', override):
                raise CommandParseError(f"Invalid variable name in llm override argument: {override}")
        elif isinstance(override, bool):
            # Boolean literal - always valid
            pass
        else:
            raise CommandParseError(f"llm override must be bool or variable name, got {type(override).__name__}: {override}")
...
class NodeModifierCommand:
    ...
    @classmethod
    def from_string(cls, modifier_str: str, comment: Optional[str] = None) -> 'NodeModifierCommand':
...
class ResamplingCommand:
    ...
    def __post_init__(self):
        ...
        if self.aggregation_function not in self.VALID_FUNCTIONS:
            raise CommandParseError(f"Invalid aggregation function: {self.aggregation_function}")
...
class ParsedCommand:
    ...
    inclusion_path: Optional[str] = None  # For @each commands
    has_multiplicity: bool = False  # True if command ends with *
    is_wildcard_assignment: bool = False  # True if uses * assignment
    
    # Scope resolution for all path components
    resolved_destination: Optional[ResolvedPath] = None
    resolved_inclusion: Optional[ResolvedPath] = None
...
class CommandParser:
    ...
    def _validate_forbidden_characters(self, command: str) -> None:
        ...
        for i, char in enumerate(command):
            if not in_string:
                if char in ('"', "'"):
                    ...
                    raise CommandParseError(f"Invalid non-ASCII character '{char}' at position {i}")
            else:
                if escaped:
                    escaped = False
                elif char == '\\':
        ...
        if forbidden_control_chars:
            raise CommandParseError(f"Invalid control characters in command: {', '.join(set(forbidden_control_chars))}")
    ...
    def _validate_newline_usage(self, command: str) -> None:
        ...
        for line_num, line in enumerate(lines):
            ...
            if line_num < len(lines) - 1 and not context_stack:
                ...
                if any(line.strip() and not line.strip().startswith('#') for line in remaining_lines):
                    raise CommandParseError("Invalid newline outside of brackets [], braces {{}}, or parentheses ()")
        
        # Ensure all contexts are properly closed  
        if context_stack:
            raise CommandParseError("Unclosed brackets, braces, or parentheses in multiline command")

    def _validate_iteration_matching(self, inclusion_path: str, variable_mappings: list[VariableMapping]) -> None:
        """
        ...
        iteration_root = inclusion_parts[0]  # e.g., "sections" from "sections.subsections"

        for mapping in variable_mappings:
            source_path = mapping.source_path

            # Skip wildcard and scope-modified paths
            if source_path == '*' or '.' in source_path and source_path.split('.')[0] in ('prompt', 'value', 'outputs', 'task'):
                continue
            ...
            if len(source_parts) > 0 and source_parts[0] != iteration_root:
                ...
                if not self._is_valid_iteration_context_access(source_path, inclusion_path):
                    raise CommandParseError(
                        f"Source path '{source_path}' must start from iteration root '{iteration_root}' "
                        f"for @each[{inclusion_path}]"
                    )

    def _is_valid_iteration_context_access(self, source_path: str, inclusion_path: str) -> bool:
        """
        ...
        iteration_collection = inclusion_parts[-1] if len(inclusion_parts) > 1 else inclusion_parts[0]  # "sections"
    ...
    def _validate_path(self, path: str, path_type: str) -> None:
        ...
        if path.startswith('.') or path.endswith('.'):
            raise CommandParseError(f"Invalid {path_type}: path cannot start or end with dots")
        
        # Check for consecutive dots
        if '..' in path:
            raise CommandParseError(f"Invalid {path_type}: path cannot contain consecutive dots")
        
        # Check for invalid characters
        if re.search(r'[^a-zA-Z0-9_.]', path):
            invalid_chars = re.findall(r'[^a-zA-Z0-9_.]', path)
            raise CommandParseError(f"Invalid {path_type}: contains invalid characters: {', '.join(set(invalid_chars))}")
        ...
        for component in components:
            ...
            if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', component):
                raise CommandParseError(f"Invalid {path_type}: '{component}' is not a valid identifier")
    ...
    def _validate_strict_whitespace(self, command: str) -> None:
        ...
        if re.search(r'[a-zA-Z_0-9]\s+\.', command):
            errors.append("Space before '.' not allowed - use 'document.sections' not 'document . sections'")
        if re.search(r'\.\s+[a-zA-Z_0-9]', command):
            errors.append("Space after '.' not allowed - use 'document.sections' not 'document. sections'")
        ...
        if re.search(r'\s@(?=each|all|resampled)', command) and not re.match(r'^!\s+@(?:each|all|resampled)', command):
        ...
        if re.search(r'(?:@each|@all|@resampled)\s+\[', command):
            errors.append("Space before '[' not allowed - use '@each[items]' not '@each [items]'")
        
        # Check for space after ]
        if re.search(r'\]\s+(-|>)', command):
            errors.append("Space after ']' not allowed - use '[items]->' not '[items] ->'")
        ...
        # Check for space before {{
        if re.search(r'\s@?\{\{', command):
            errors.append("Space before '{{' not allowed - use 'task@{{' not 'task @{{'")
        ...
        if errors:
            raise CommandParseError(f"Invalid command syntax - strict whitespace violations: {'; '.join(errors)}")
    
    def parse(self, command: str) -> 'VariableAssignmentCommand | ExecutionCommand | ResamplingCommand | NodeModifierCommand | ParsedCommand':
        """
        ...
        if (re.match(r'^!\s*[^=@()]*=', command) and '@' not in command and '->' not in command and
            ...
            if var_name_match:
                ...
                if value_match:
                    ...
                    if not (value.startswith('"') or value.startswith("'") or 
                        ...
                        raise CommandParseError(f"Unquoted value must be a number or boolean, got: {value}")
        ...
        if re.match(r'^!\s*@resampled\s*\[', command):
            ...
            func_match = re.match(r'^!\s*@resampled\s*\[\s*[^\]]*\s*\]\s*->\s*(\S*)(?:\s*#.*)?$', command)
            if func_match:
                func_name = func_match.group(1)
                if not func_name:
                    raise CommandParseError("Empty aggregation function in resampling command")
                valid_functions = ResamplingCommand.VALID_FUNCTIONS
                if func_name not in valid_functions:
                    raise CommandParseError(f"Invalid aggregation function: {func_name}")
        ...
        raise CommandParseError(f"Invalid command syntax - malformed command: {command}")
    ...
    def _parse_execution_command(self, match) -> ExecutionCommand:
        ...
        positional_args, named_args = self._parse_execution_arguments(arguments_str) if arguments_str else ([], {})
    ...
    def _parse_traditional_command(self, match) -> ParsedCommand:
        ...
        is_wildcard_assignment = any(mapping.source_path == '*' for mapping in variable_mappings)
        
        # Validate wildcard usage
        if is_wildcard_assignment:
            if command_type == CommandType.EACH:
                raise CommandParseError("@each commands cannot use wildcard (*) in variable mappings")

            # Check for mixed wildcard/non-wildcard usage (not allowed)
            if len(variable_mappings) > 1:
                wildcard_count = sum(1 for mapping in variable_mappings if mapping.source_path == '*')
                non_wildcard_count = len(variable_mappings) - wildcard_count

                if wildcard_count > 0 and non_wildcard_count > 0:
                    raise CommandParseError("Cannot mix wildcard (*) with non-wildcard mappings in the same command")
    ...
    def _parse_value(self, value_str: str) -> str | int | float | bool:
        ...
        try:
            ...
            raise CommandParseError(f"Unquoted value must be a number or boolean, got: {value_str}")
    ...
    def _parse_execution_arguments(self, args_str: str) -> tuple[List[str | int | float | bool], dict[str, str | int | float | bool]]:
        """
    ...
    def _parse_variable_mappings(self, mappings_str: str) -> List[VariableMapping]:
        ...
        for part in mapping_parts:
            ...
            if explicit_match:
                ...
                if not target:
                    raise CommandParseError("Invalid command syntax - empty variable name in mapping")
                if source == '':  # Empty source should fail
                    raise CommandParseError("Invalid command syntax - empty source path in mapping")
            ...
            raise CommandParseError(f"Invalid command syntax - malformed variable mapping: {part}")
    ...
    def calculate_iterable_depth(self, node, path_components: List[str]) -> int:
        ...
        for component in path_components:
            ...
            try:
                ...
                raise CommandParseError(f"Cannot analyze type hints for {current_node_type.__name__}")

            if component not in hints:
                raise CommandParseError(f"Field '{component}' does not exist in {current_node_type.__name__}")
            ...
            if origin in (list, dict, set, tuple):
                ...
                if args:
                    ...
                    try:
                        if hasattr(element_type, '__mro__') and issubclass(element_type, PromptTreeNode):
    ...
    def _is_proper_subchain(self, inclusion_path: List[str], rhs_path: List[str]) -> bool:
        """
    ...
    def validate_last_matching_iterable(self, node, inclusion_path: List[str], rhs_paths: List[List[str]]) -> dict:
        """
        ...
        if inclusion_depth == 0:
            raise CommandParseError("@each requires at least one iterable in inclusion path")
        ...
        try:
            ...
            for rhs_path in rhs_paths:
                try:
                    ...
                    return {"valid": False, "error": f"RHS path validation failed: {str(e)}", "has_complete_coverage": False}

            # Step 2: Check for complete coverage - at least one RHS must match inclusion depth
            has_complete_coverage = any(rhs_depth >= inclusion_depth for rhs_depth in rhs_depths)
            ...
            subchain_errors = self._validate_proper_subchains(node, inclusion_path, rhs_paths)
            if subchain_errors:
                return {
                    "valid": False,
            ...
            return {"valid": False, "error": f"Validation error: {str(e)}", "has_complete_coverage": False}

    def _validate_proper_subchains(self, node, inclusion_path: List[str], rhs_paths: List[List[str]]) -> List[str]:
        """
        ...
        try:
            inclusion_iterable_count = self.calculate_iterable_depth(node, inclusion_path)
            if inclusion_iterable_count == 0:
                errors.append(f"Inclusion path {'.'.join(inclusion_path)} must contain at least one iterable field")
                return errors  # Can't proceed with further validation
        except Exception as e:
            errors.append(f"Cannot validate inclusion path {'.'.join(inclusion_path)}: {str(e)}")
            return errors

        # Check if inclusion path starts with iterable
        try:
            first_field_iterable = self.calculate_iterable_depth(node, inclusion_path[:1]) > 0
            if not first_field_iterable:
                errors.append(f"Inclusion path {'.'.join(inclusion_path)} must start with an iterable field")
        except Exception:
        ...
        try:
            ...
            if len(inclusion_path) > 1:
                ...
                if not last_field_adds_iterable:
                    errors.append(f"Inclusion path {'.'.join(inclusion_path)} must end with an iterable field")
        except Exception:
            pass  # Field validation error will be caught elsewhere
        ...
        for i, rhs_path in enumerate(rhs_paths):
            try:
                ...
                if common_prefix_len > 0:
                    shared_iterable_count = self.calculate_iterable_depth(node, inclusion_path[:common_prefix_len])
                else:
                    shared_iterable_count = 0

                # RHS must share at least 1 iterable field with inclusion path
                if shared_iterable_count == 0:
                    errors.append(f"Source path {'.'.join(rhs_path)} must start from iteration root '{'.'.join(inclusion_path)}' - RHS paths must share at least 1 iterable field with inclusion path")
            ...
            if len(rhs_remaining) == 0:
                ...
                for k in range(len(inclusion_remaining)):
                    if k >= len(rhs_remaining) or rhs_remaining[k] != inclusion_remaining[k]:
                ...
                if not is_valid_subchain:
                    ...
                    if len(inclusion_remaining) == 1 and len(rhs_remaining) == 1:
                        ...
                        try:
                            ...
                            divergent_depth = self.calculate_iterable_depth(node, divergent_path)
                            parent_depth = self.calculate_iterable_depth(node, parent_path)
                            ...
                            if divergent_depth == parent_depth:
                                ...
                                errors.append(f"Iterable path mismatch: source path {'.'.join(rhs_path)} accesses different iterable '{rhs_remaining[0]}' than inclusion path '{'.'.join(inclusion_path)}' which requires '{inclusion_remaining[0]}'")
                                continue
                        except Exception:
                    ...
                    divergent_component = rhs_remaining[0] if rhs_remaining else "<empty>"
                    expected_component = inclusion_remaining[0] if inclusion_remaining else "<empty>"
                    errors.append(f"Subchain mismatch: source path {'.'.join(rhs_path)} diverges from inclusion path {'.'.join(inclusion_path)} - expected '{expected_component}' but found '{divergent_component}' (paths must be proper subchains, not siblings)")

        return errors


def parse_command(command: str) -> 'VariableAssignmentCommand | ExecutionCommand | ResamplingCommand | NodeModifierCommand | ParsedCommand':
    """
