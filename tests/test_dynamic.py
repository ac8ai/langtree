...
from langtree.dynamic import (
    describe_datafield_type,
    get_type,
    resolve_type,
    schema_to_model,
)
...
def test_get_type_union_types():
    t = get_type("int | None")
    assert get_args(t) == (int, type(None)), (
        "Expected 'int | None' to resolve to a union of int and NoneType"
    )
...
def test_get_type_pydantic_types():
    ...
    assert get_type("SimpleModel", custom_types) is SimpleModel, (
        "Expected 'SimpleModel' to resolve to SimpleModel"
    )
    assert get_type("ComplexModel", custom_types) is ComplexModel, (
        "Expected 'ComplexModel' to resolve to ComplexModel"
    )
...
def test_resolve_type_any_of():
    ...
    t = resolve_type(field)
    assert get_args(t) == (str, type(None)), (
        "Expected 'anyOf' to resolve to a union of str and NoneType"
    )
...
def test_resolve_type_pydantic_any_of():
    field = {"anyOf": [{"type": "SimpleModel"}, {"type": "ComplexModel"}]}
    custom_types = {
        "SimpleModel": SimpleModel,
        "ComplexModel": ComplexModel,
    }
    t = resolve_type(field, custom_types)
    assert get_args(t) == (SimpleModel, ComplexModel), (
        "Expected 'anyOf' to resolve to a union of SimpleModel and ComplexModel"
    )
...
def test_describe_datafield_type():
    ...
    assert description == "`str`", (
        "Expected description for 'str' type to be '`string`'. Got: " + description
    )

    field = {"type": "int | None"}
    description = describe_datafield_type(field)
    assert description == "`int | None`", (
        "Expected description for 'int | None' to be '`int | None`'. Got: "
        + description
    )

    field = {"anyOf": [{"type": "string"}, {"type": "int | None"}]}
    description = describe_datafield_type(field)
    assert description == "`str | int | None`", (
        "Expected description for 'anyOf' to be '`str | int | None`. Got: "
        + description
    )
...
def test_schema_to_model_simple():
    ...
    assert schema == model.model_json_schema(), (
        "Expected schema to match the model's JSON schema"
    )
