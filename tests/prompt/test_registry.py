...
from langtree.prompt import (
    RunStructure, VariableRegistry, PendingTargetRegistry, get_scope
)
...
class TestVariableRegistry:
    ...
    def test_relationship_types(self):
        ...
        registry.register_variable("title", get_scope("value"), "task.test", "all", False)
        
        # 1:n relationship (all command, has multiplicity) 
        registry.register_variable("results", get_scope("outputs"), "task.test", "all", True)
        
        # n:n relationship (each command, has multiplicity)
        registry.register_variable("items", get_scope("value"), "task.test", "each", True)
