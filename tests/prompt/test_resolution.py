...
from langtree.prompt import RunStructure, PromptTreeNode, StructureTreeNode
from langtree.prompt.exceptions import NodeTagValidationError, PathValidationError, RuntimeVariableError
...
class TestIntegrationWorkflow:
    ...
    def test_command_processing_calls_resolution(self):
        ...
        pytest.skip("TODO: Fix variable registration - DPCL variable targets should be tracked per LANGUAGE_SPECIFICATION.md")
    ...
    def test_pending_target_resolution_completion(self):
        ...
        assert final_command_count == initial_command_count  # Command should already be there from initial processing
    ...
    def test_pending_target_multiple_commands_same_target(self):
        ...
        pending_commands = run_structure._pending_target_registry.pending_targets["task.shared"]
    ...
    def test_pending_target_nested_path_resolution(self):
        ...
        assert "task.analysis.deep.nested" in run_structure._pending_target_registry.pending_targets
    ...
    def test_pending_target_no_source_node_edge_case(self):
        ...
        from langtree.commands.parser import ParsedCommand, CommandType
    ...
    def test_pending_target_duplicate_command_prevention(self):
        ...
        pending_targets = run_structure._pending_target_registry.resolve_pending("task.later")
    ...
    def test_pending_target_partial_path_resolution(self):
        ...
        if "task.analysis.section" not in pending_targets:
            ...
            assert len(pending_targets) == 0, f"Unexpected pending targets: {list(pending_targets.keys())}"
    ...
    def test_scope_routing_integration(self):
        ...
        with pytest.raises(RuntimeVariableError, match=r"cannot contain dots"):
            resolve_runtime_variables(content_with_dots, self.run_structure, source_node)

        # Test that task scope syntax is rejected (users cannot use dots)
        content_task_scope = "Task data: {task.source.input_data}"
        with pytest.raises(RuntimeVariableError, match=r"cannot contain dots"):
            resolve_runtime_variables(content_task_scope, self.run_structure, source_node)
...
class TestCurrentNodeContextResolution:
    ...
    def test_resolve_simple_path_that_exists(self):
        ...
        result = self.run_structure._resolve_in_current_node_context("sections", "task.analysis")
    ...
    def test_resolve_nested_path_that_exists(self):
        ...
        result = self.run_structure._resolve_in_current_node_context("metadata.type", "task.analysis")
    ...
    def test_resolve_path_that_does_not_exist(self):
        ...
        with pytest.raises((KeyError, AttributeError)):
            self.run_structure._resolve_in_current_node_context("nonexistent_field", "task.analysis")
...
class TestScopeSegmentContextResolution:
    ...
    def test_route_to_current_node_scope(self):
        ...
        result = self.run_structure._resolve_scope_segment_context("input_field", "task.complex")
    ...
    def test_route_to_value_scope(self):
        ...
        result = self.run_structure._resolve_scope_segment_context("value.input_field", "task.complex")
    ...
    def test_route_to_outputs_scope(self):
        ...
        result = self.run_structure._resolve_scope_segment_context("outputs.process_status", "task.complex")
    ...
    def test_route_to_task_scope(self):
        ...
        result = self.run_structure._resolve_scope_segment_context("task.complex", "task.complex")
        ...
        with pytest.raises(KeyError):
            self.run_structure._resolve_scope_segment_context("task.nonexistent_field", "task.complex")
        ...
        try:
            result = self.run_structure._resolve_scope_segment_context("task.nonexistent", "task.source")
    ...
    def test_handle_invalid_scope(self):
        ...
        with pytest.raises(KeyError):
            self.run_structure._resolve_scope_segment_context("invalid.field", "task.complex")
...
class TestCurrentPromptContextResolution:
    ...
    def test_resolve_prompt_path(self):
        ...
        result = self.run_structure._resolve_in_current_prompt_context("prompt_template", "task.prompt")
        ...
        # For now, just verify the method doesn't crash
        result = self.run_structure._resolve_in_current_prompt_context("template_var", "task.source")
        # Accept placeholder implementation
        assert isinstance(result, str)
    
    def test_resolve_prompt_variables(self):
        """Test resolving prompt variables."""
        result = self.run_structure._resolve_in_current_prompt_context("variables.data", "task.prompt")
        ...
        # For now, just verify the method doesn't crash
        result = self.run_structure._resolve_in_current_prompt_context("nonexistent_var", "task.source")
        # Accept placeholder implementation
        assert isinstance(result, str)
    ...
    def test_handle_invalid_prompt_path(self):
        ...
        result = self.run_structure._resolve_in_current_prompt_context("nonexistent", "task.prompt")
        ...
        # For now, just verify the method doesn't crash  
        try:
            result = self.run_structure._resolve_in_current_prompt_context("deeply.invalid.path", "task.source")
            # Accept placeholder implementation
            assert isinstance(result, str)
        except KeyError:
...
class TestTaskScopeContextResolution:
    ...
    def test_resolve_task_reference(self):
        ...
        result = self.run_structure._resolve_in_global_tree_context("task.source.source_data")
    ...
    def test_handle_nonexistent_task(self):
        ...
        with pytest.raises(KeyError, match="Path 'task.nonexistent_task.field' not found in global tree"):
            self.run_structure._resolve_in_global_tree_context("task.nonexistent_task.field")
    
    def test_handle_invalid_task_path(self):
        """Test handling invalid task path format."""
        with pytest.raises(ValueError):
            self.run_structure._resolve_in_global_tree_context("")  # Empty path should fail
...
class TestTargetNodeContextResolution:
    ...
    def test_validate_nonexistent_structure_path(self):
        ...
        with pytest.raises((KeyError, AttributeError, ValueError)):
            self.run_structure._resolve_in_target_node_context("nonexistent_field", self.target_node)
...
class TestValueContextResolution:
    ...
    def test_resolve_value_path(self):
        ...
        result = self.run_structure._resolve_in_value_context("input_data", "task.value")
        ...
        try:
            result = self.run_structure._resolve_in_value_context("input_data", "task.value")  # Use existing field
...
class TestOutputsContextResolution:
    ...
    def test_resolve_outputs_path(self):
        ...
        result = self.run_structure._resolve_in_outputs_context("result", "task.outputs")
        assert result is None
        
        # Test case with no outputs during chain assembly
        result_no_outputs = self.run_structure._resolve_in_outputs_context("result", "task.nonexistent")
...
class TestGlobalTreeContextResolution:
    ...
    def test_resolve_global_path(self):
        ...
        result = self.run_structure._resolve_in_global_tree_context("task.parent.parent_data")
...
class TestRuntimeVariableSystemSpecCompliance:
    ...
    def test_basic_runtime_variable_syntax_compliance(self):
        ...
        assert hasattr(run_structure, 'resolve_runtime_variable'), "resolve_runtime_variable method missing from RunStructure"
        ...
        if node:
            ...
            assert expanded == "Field value: {prompt__runtime_vars__field}", "Runtime variable should expand to double underscore format"
    ...
    def test_assembly_variable_runtime_separation(self):
        ...
        assert expanded_field == "Using model: {prompt__with_separation__field_model}"  # Should expand, not resolve to value
        ...
        assert "assembly variable" in error_msg.lower() or "undefined" in error_msg.lower()
...
class TestSpecificationViolationDetection:
    ...
    def test_detect_invalid_command_combinations(self):
        ...
        with pytest.raises(CommandParseError, match=r"Invalid command syntax|malformed"):
    ...
    def test_detect_template_variable_violations(self):
        ...
        with pytest.raises((TemplateVariableError, ValueError), match=r"spacing|template|variable"):
...
class TestPerformanceAndStressEdgeCases:
    ...
    def test_massive_command_count_resolution(self):
        ...
        commands = [f"! @all->task.target_{i}@{{value.result_{i}=field}}" for i in range(500)]
...
class TestSpecificationContractEnforcement:
    ...
    @pytest.mark.skip(reason="Assembly variables are not available at runtime - strict separation enforced")
    ...
    @pytest.mark.skip(reason="TODO: Implement Assembly Variable with complex data types")
...
class TestCrossModuleIntegrationCompliance:
    ...
    @pytest.mark.skip(reason="TODO: Implement template_variables.py + resolution.py integration")
    ...
    @pytest.mark.skip(reason="TODO: Implement validation.py integration with all modules")
...
class TestRuntimeVariableErrorHandlingCompliance:
    ...
    def test_undefined_runtime_variable_handling(self):
        ...
        result = resolve_runtime_variables("Value: {defined_field}", run_structure, node)
...
class TestSpecificationComplianceEdgeCases:
    ...
    @pytest.mark.skip(reason="TODO: Test scope system compliance from COMPREHENSIVE_GUIDE.md")
    ...
    @pytest.mark.skip(reason="TODO: Test execution command compliance from LANGUAGE_SPECIFICATION.md")
    ...
    def test_nested_field_access(self):
        ...
        result = run_structure._resolve_in_global_tree_context("task.complex.data.nested.value")
        assert result == "test"
        
        # Test list access
        result = run_structure._resolve_in_global_tree_context("task.complex.data.numbers[0]")
        assert result == 1
        
        # Test metadata access
        result = run_structure._resolve_in_global_tree_context("task.complex.metadata.type")
        assert result == "test"
    ...
    def test_cross_node_variable_resolution(self):
        ...
        class TaskTarget(PromptTreeNode):
            ...
            timeout: str = "{{task.source.shared_config.timeout}}"  # Will be resolved to int
    ...
    def test_scope_resolution_validation(self):
        ...
        result = run_structure._resolve_in_current_node_context("local_var", "task_scoped")
        assert result == "local_value"
        
        # Test that nested access works correctly
        result = run_structure._resolve_in_current_node_context("nested.local_var", "task_scoped")
    ...
    def test_runtime_variable_error_handling(self):
        ...
        with pytest.raises((KeyError, AttributeError, ValueError)):
            run_structure._resolve_in_current_node_context("non_existent", "task.simple")
    ...
    def test_dynamic_field_access(self):
        ...
        result = run_structure._resolve_in_current_node_context("computed_field", "task_dynamic")
        assert result == "computed_test"
        
        # Test accessing base field used in computation
        result = run_structure._resolve_in_current_node_context("base_value", "task_dynamic")
        assert result == "test"
    ...
    def test_runtime_variable_caching(self):
        ...
        result1 = run_structure._resolve_in_current_node_context("expensive_computation", "task.expensive")

        # Second access - properties are computed each time (no caching currently implemented)
        result2 = run_structure._resolve_in_current_node_context("expensive_computation", "task.expensive")
    ...
    def test_runtime_variable_type_conversion(self):
        ...
        assert run_structure._resolve_in_current_node_context("string_value", "task.typed") == "123"
        assert run_structure._resolve_in_current_node_context("int_value", "task.typed") == 456
        assert run_structure._resolve_in_current_node_context("float_value", "task.typed") == 78.9
        assert run_structure._resolve_in_current_node_context("bool_value", "task.typed") is True
        assert run_structure._resolve_in_current_node_context("list_value", "task.typed") == [1, 2, 3]
        assert run_structure._resolve_in_current_node_context("dict_value", "task.typed") == {"key": "value"}
    
    @pytest.mark.skip(reason="TODO: Implement runtime variable with command integration")
...
class TestContextResolutionImplementation:
    ...
    def test_resolve_variable_mapping_wildcard_source(self):
        ...
        result = _resolve_variable_mapping_context(rs, mock_target, mock_source, "task.source", None)
    ...
    def test_resolve_variable_mapping_outputs_scope(self):
        ...
        from langtree.prompt.structure import StructureTreeNode, PromptTreeNode
        ...
        result = _resolve_variable_mapping_context(rs, mock_target, mock_source, "task.source", target_node)
...
class TestRuntimeVariableResolution:
    ...
    def test_basic_runtime_variable_resolution(self):
        ...
        resolved_content = resolve_runtime_variables(content_with_vars, self.run_structure, task_node)
    ...
    def test_scoped_runtime_variable_resolution(self):
        ...
        content_with_vars = "Task: {name} with priority {priority}\nTimestamp: {timestamp}"
    ...
    def test_scope_aware_runtime_variable_resolution(self):
        ...
        node_result = self.run_structure._resolve_in_current_node_context("name", "task.with_scoped_vars")
        assert node_result == "test_task"

        # Test prompt context resolution - access prompt-related data
        prompt_result = self.run_structure._resolve_in_current_prompt_context("context", "task.with_scoped_vars")
        assert prompt_result is not None

        # Test value context resolution - access value data
        value_result = self.run_structure._resolve_in_value_context("data", "task.with_scoped_vars")
        assert value_result is not None

        # Test outputs context resolution - should return None during chain assembly
        outputs_result = self.run_structure._resolve_in_outputs_context("result", "task.with_scoped_vars")
...
class TestDoubleUnderscoreExpansion:
    ...
    def test_simple_runtime_variable_expansion(self):
        ...
        assert result == "Hello {prompt__analyzer__model_name} and {prompt__analyzer__temperature}"
    ...
    def test_nested_node_path_expansion(self):
        ...
        assert result == "Using {prompt__analytics__deep__processor__step1__config_value} for analysis"
    ...
    def test_runtime_variables_with_double_underscores_rejected(self):
        ...
        with pytest.raises(RuntimeVariableError, match="cannot contain double underscores"):
...
class TestCWDPathResolution:
    ...
    def test_variable_mapping_with_cwd(self):
        ...
        mapping = PathResolver.resolve_variable_mapping_with_cwd("summary", "task.data.source", cwd)
    ...
    def test_deeply_nested_cwd_resolution(self):
        ...
        assert result.scope_modifier is None
        assert result.path_remainder == "task.analytics.deep.processor.final_step.output"
...
class TestIterationMatchingValidation:
    ...
    def test_valid_iteration_source_paths(self):
        ...
        cmd = parser.parse('! @each[sections.subsections]->task@{{value.items=sections.subsections}}*')
        assert cmd.inclusion_path == "sections.subsections"

        # Valid: Field access from iteration items
        cmd = parser.parse('! @each[sections.subsections]->task@{{value.content=sections.subsections.text}}*')
        assert cmd.inclusion_path == "sections.subsections"

        # Valid: Parent field access
        cmd = parser.parse('! @each[sections.subsections]->task@{{value.title=sections.title}}*')
        assert cmd.inclusion_path == "sections.subsections"
    ...
    def test_invalid_iteration_source_paths(self):
        ...
        from langtree.commands.parser import CommandParseError
        with pytest.raises(CommandParseError, match="must start from iteration root 'sections'"):
    ...
    def test_scope_modified_paths_bypass_validation(self):
        ...
        cmd = parser.parse('! @each[sections.subsections]->task@{{value.items=sections.subsections, prompt.data=task.unrelated.path}}*')
        assert len(cmd.variable_mappings) == 2

        cmd = parser.parse('! @each[sections.subsections]->task@{{value.items=sections.subsections, outputs.result=task.other}}*')
        assert len(cmd.variable_mappings) == 2

    def test_deep_iteration_matching(self):
        """Test iteration matching with deeply nested iteration paths."""
        from langtree.commands.parser import CommandParser, CommandParseError

        parser = CommandParser()

        # Valid: Deep iteration with matching source
        cmd = parser.parse('! @each[docs.chapters.sections]->task@{{value.content=docs.chapters.sections.text}}*')
        assert cmd.inclusion_path == "docs.chapters.sections"

        # Valid: Parent level access in deep iteration
        cmd = parser.parse('! @each[docs.chapters.sections]->task@{{value.title=docs.chapters.title}}*')
        assert cmd.inclusion_path == "docs.chapters.sections"

        # Invalid: Wrong root in deep iteration
        with pytest.raises(CommandParseError, match="must start from iteration root 'docs'"):
            parser.parse('! @each[docs.chapters.sections]->task@{{value.content=chapters.sections.text}}*')
    ...
    def test_single_level_iteration(self):
        ...
        with pytest.raises(CommandParseError, match="must start from iteration root 'items'"):
    ...
    def test_wildcard_paths_bypass_validation(self):
        ...
        with pytest.raises(CommandParseError, match="@each commands cannot use wildcard"):
    ...
    def test_mixed_valid_invalid_mappings(self):
        ...
        with pytest.raises(CommandParseError, match="must start from iteration root 'sections'"):
            parser.parse('! @each[sections.subsections]->task@{{value.items=sections.subsections, value.other=wrong.path}}*')