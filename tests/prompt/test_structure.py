...
class TestTemplateVariableSystem:
    ...
    def test_prompt_subtree_automatic_addition(self):
        ...
        print(f"Does it contain PROMPT_SUBTREE? {'{PROMPT_SUBTREE}' in task_node.clean_docstring}")
    ...
    def test_prompt_subtree_resolution(self):
        ...
        assert "{PROMPT_SUBTREE}" not in resolved_content, "Template variable should be resolved"
        
        # Check that field titles and descriptions are included
        assert "field1" in resolved_content.lower() or "field 1" in resolved_content.lower()
        assert "field2" in resolved_content.lower() or "field 2" in resolved_content.lower()
    ...
    def test_collected_context_automatic_fallback(self):
        ...
        self.structure.add(TaskTargetProcess)        # TODO: Implement automatic context fallback
    ...
    def test_template_variable_resolution_order(self):
        ...
        self.structure.add(TaskComplexTemplate)        # TODO: Implement template resolution order
...
class TestTemplateVariableValidation:
    ...
    @pytest.mark.xfail(reason="Template variable validation not yet implemented - invalid names should be rejected")
...
class TestPendingTargetResolutionCore:
    ...
    def test_basic_forward_reference_resolution_workflow(self):
        ...
        assert "task.later" in self.run_structure._pending_target_registry.pending_targets
...
class TestPendingTargetEdgeCasesAdvanced:
    ...
    def test_multiple_commands_same_pending_target_advanced(self):
        ...
        assert "task.shared" in self.run_structure._pending_target_registry.pending_targets
        pending_commands = self.run_structure._pending_target_registry.pending_targets["task.shared"]
...
class TestPendingTargetIntegrationWithResolution:
    ...
    def test_multiple_forward_references_to_same_node_batch_resolution(self):
        ...
        assert len(self.run_structure._pending_target_registry.pending_targets) == 1
        assert "task.late_target" in self.run_structure._pending_target_registry.pending_targets
        pending_commands = self.run_structure._pending_target_registry.pending_targets["task.late_target"]
    ...
    @pytest.mark.skip("TODO: Implement post-processing verification with observable effects")
