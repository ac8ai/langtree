...
import pytest
from langtree.prompt import RunStructure, PromptTreeNode
...
class TestAssemblyRuntimeSeparation:
    ...
    def test_runtime_variables_work_correctly(self):
        ...
        assert expanded == "Title: {prompt__with_runtime_context__title}, Content: {prompt__with_runtime_context__content}"
...
class TestRuntimeVariableResolutionPriority:
    ...
    def test_current_node_context_priority(self):
        ...
        assert expanded == "{prompt__with_context__title} - {prompt__with_context__description}"
...
class TestArchitecturalIntegrity:
    ...
    def test_no_assembly_runtime_bridging_in_parser(self):
        ...
        expanded = resolve_runtime_variables("Runtime: {runtime_field}", structure, node)
    ...
    def test_documentation_compliance(self):
        ...
        assert expanded == "Analysis: {prompt__documentation_example__analysis}, Confidence: {prompt__documentation_example__confidence}"
...
class TestErrorMessageQuality:
    ...
    def test_valid_variable_syntax_expansion(self):
        ...
        class TaskForBridgingTest(PromptTreeNode):
            ...
            legacy_field: str = "old_value"  # Use regular field instead of assembly variable
...
class TestCompleteVariableTypeCoverage:
    ...
    def test_field_references_for_resampling(self):
        ...
        # Should expand to double underscore format
        assert expanded == "Priority field: {prompt__with_field_references__priority}, Status field: {prompt__with_field_references__status}"
...
class TestRuntimeVariableResolutionPriorityComplete:
    ...
    def test_resolution_priority_chain(self):
        ...
        class TaskForPriorityTesting(PromptTreeNode):
            ...
            current_field: str = "current_node_value"  # Priority 1: Current Node Context
        ...
        pytest.skip("TODO: Implement tests for complete priority chain (Task, Outputs, Value, Prompt contexts) - only Current Node context is tested")
    ...
    def test_context_type_separation(self):
        ...
        assert expanded == "{prompt__with_multiple_contexts__title} | {prompt__with_multiple_contexts__content} | {prompt__with_multiple_contexts__metadata}"
    ...
    def test_all_variables_expand_in_priority_chain(self):
        ...
        assert expanded == "Known: {prompt__for_undefined_testing__known}, Unknown: {prompt__for_undefined_testing__unknown_variable}"
...
class TestSpecificationCompliance:
    ...
    def test_assembly_vs_runtime_separation_specification(self):
        ...
        with pytest.raises(RuntimeVariableError) as exc_info:
            resolve_runtime_variables("Mixed: {config_value} and {runtime_field}", structure, node)
        ...
        expanded = resolve_runtime_variables(content, structure, node)
        assert expanded == "Runtime: {prompt__specification_example__runtime_field}, Status: {prompt__specification_example__status}"
    ...
    def test_variable_type_taxonomy_compliance(self):
        ...
        class TaskTaxonomyExample(PromptTreeNode):
            ...
            """
            runtime_field: str = "runtime_value"  # Type 2: Runtime Variables ({runtime_field})
        ...
        expanded = resolve_runtime_variables("Collection: {collection}", structure, node)
        assert expanded == "Collection: {prompt__taxonomy_example__collection}"
    
    @pytest.mark.skip(reason="TODO: Implement assembly variable conflict detection - specification requires strict prohibition")
    def test_conflict_prohibition_compliance(self):
        ...
        from langtree.prompt.exceptions import (
            FieldValidationError, VariableTargetValidationError
        )
        ...
        with pytest.raises((FieldValidationError, VariableTargetValidationError, ValueError)) as exc_info:
            structure.add(TaskConflictingVariableField)

        error_msg = str(exc_info.value).lower()
        assert any(term in error_msg for term in ["conflict", "field_name", "already exists", "duplicate"]), \
        ...
        error_msg = str(exc_info.value).lower()
        assert any(term in error_msg for term in ["duplicate", "already", "existing", "reassignment"]), \
    ...
    def test_documentation_example_patterns(self):
        ...
        assert expanded == "Analysis: {prompt__documentation_pattern__analysis}, Confidence: {prompt__documentation_pattern__confidence}"
