    ...
    def test_aggressive_chain_structure_introspection(self):
        ...
        try:
            ...
            if hasattr(chain, "steps"):
                ...
                assert len(steps) >= 3, (
                    f"Linear chain should have at least 3 steps, got {len(steps)}"
                )
                ...
                for i, step in enumerate(steps):
                    ...
                    assert hasattr(step, "invoke"), (
                        f"Step {i} ({step_type}) should be invokable"
                    )
            ...
            try:
                ...
                assert len(graph.nodes) >= 3, (
                    f"Graph should have at least 3 nodes for linear chain, got {len(graph.nodes)}"
                )

                # Verify graph connectivity
                assert len(graph.edges) >= 2, (
                    f"Graph should have at least 2 edges for linear chain, got {len(graph.edges)}"
                )

                # Test graph visualization
                try:
                    mermaid = graph.draw_mermaid()
                    assert isinstance(mermaid, str) and len(mermaid) > 100, (
                        "Should generate substantial mermaid diagram"
                    )
                    print(f"DEBUG: Generated mermaid diagram ({len(mermaid)} chars)")

                    # Mermaid should contain references to our tasks
                    task_refs_in_mermaid = sum(
                        1
                        for task in ["start", "middle", "end"]
                        if task in mermaid.lower()
                    )
                    assert task_refs_in_mermaid >= 2, (
                        f"Mermaid should reference our tasks, found {task_refs_in_mermaid} references"
                    )

                except Exception as graph_error:
                    print(
                        f"DEBUG: Graph visualization failed (acceptable): {graph_error}"
                    )
            ...
            assert "chain_steps" in execution_plan, (
                "Execution plan must have chain_steps"
            )

            steps = execution_plan["chain_steps"]
            assert len(steps) == 3, (
                f"Should have exactly 3 steps for linear chain, got {len(steps)}"
            )
            ...
            for fragment in expected_fragments:
                ...
                assert len(matching_steps) >= 1, (
                    f"Should have step matching '{fragment}', got tags: {step_tags}"
                )

            # AGGRESSIVE TEST 5: Variable flow introspection
            assert "variable_flows" in execution_plan, (
                "Execution plan must have variable_flows"
            )

            flows = execution_plan["variable_flows"]
            assert len(flows) >= 2, (
                f"Linear chain should have at least 2 variable flows, got {len(flows)}"
            )
            ...
            for i, flow in enumerate(flows):
                ...
                for field in required_fields:
                    assert field in flow, (
                        f"Flow {i} missing required field '{field}': {flow}"
                    )
                ...
                assert from_ref and len(str(from_ref)) > 0, (
                    f"Flow {i} has empty source reference: {flow}"
                )
                assert to_ref and len(str(to_ref)) > 0, (
                    f"Flow {i} has empty target reference: {flow}"
                )
            ...
            if "prompt_content" in execution_plan:
                ...
                for step_tag, content in prompt_content.items():
                    ...
                    assert len(content) > 50, (
                        f"Prompt for {step_tag} should be substantial, got {len(content)} chars"
                    )

                    # Check for template variable processing
                    remaining_templates = [
                        var
                        for var in ["{PROMPT_SUBTREE}", "{COLLECTED_CONTEXT}"]
                        if var in content
                    ]
                    if remaining_templates:
                        print(
                            f"DEBUG: Unprocessed templates in {step_tag}: {remaining_templates}"
                        )
            ...
            if hasattr(chain, "first") and hasattr(chain, "last"):
                print(
                    f"DEBUG: Chain appears to be a sequence with first={type(chain.first).__name__}, last={type(chain.last).__name__}"
                )
            ...
            if hasattr(chain, "steps__"):
                ...
                assert task_matches >= 2, (
                    f"Step names should reflect our tasks, got: {step_names}"
                )
            ...
            if any(expected in error_msg for expected in expected_errors):
                ...
                pytest.skip(
                    f"Chain building failed with expected validation/setup error: {e}"
                )
            else:
