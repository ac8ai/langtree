...
from langtree.prompt import (
    RunStructure, PromptTreeNode, get_scope
)
...
class TestErrorHandling:
    ...
    def test_validation_methods(self):
        ...
        assert any("task.missing_target" in target for target in validation['unresolved_targets'])
        assert any("task.another_missing" in target for target in validation['unresolved_targets'])
    ...
    def test_unsatisfied_variable_detection(self):
        ...
        registry.add_satisfaction_source("satisfied_var", get_scope("value"), "some_source")
...
class TestRealWorldComplexity:
    ...
    def test_missing_source_fields_edge_case(self):
        ...
        with pytest.raises(VariableSourceValidationError, match=r"Source field.*missing_field.*does not exist"):
    ...
    def test_partially_missing_structure_edge_case(self):
        ...
        with pytest.raises(VariableSourceValidationError, match=r"Source field.*documents.title.*does not exist"):
    ...
    def test_completely_missing_iteration_edge_case(self):
        ...
        with pytest.raises(FieldValidationError, match=r"inclusion field.*missing_sections.*does not exist"):
...
class TestDeferredContextResolution:
    ...
    def test_complex_path_resolution(self):
        ...
        class TaskComplexPaths(PromptTreeNode):
            ...
            class DataStructure(PromptTreeNode):
                items: list[DataItem] = [DataItem(content="Item 1"), DataItem(content="Item 2")]
    ...
    def test_wildcard_mapping_handling(self):
        ...
        wildcard_var = next((v for v in prompt_vars if "all_data" in v.variable_path), None)
        if wildcard_var:
    ...
    def test_scope_resolution_across_contexts(self):
        ...
        outputs_vars = [v for v in variables.values() if v.get_scope_name() == "outputs"]
...
class TestExecutionPlanGeneration:
    ...
    def test_execution_plan_with_external_inputs(self):
        ...
        class TaskExternalInputs(PromptTreeNode):
            ...
            unsatisfied_source: str = Field(default="test", description="! @->task.external_summarizer@{{prompt.summary=unsatisfied_source}}")
        ...
        unsatisfied_sources = [flow['from'] for flow in variable_flows
                              if flow['from'] == 'unsatisfied_source']
        assert len(unsatisfied_sources) > 0, "Should identify 'unsatisfied_source' in variable flows"
    ...
    def test_execution_plan_variable_flows(self):
        ...
        class TaskVariableFlow(PromptTreeNode):
            ...
            document_title: str = Field(default="Test Title", description="! @->task.processor@{{prompt.title=document_title}}")
            document_content: str = Field(default="Test Content", description="! @->task.summarizer@{{value.content=document_content}}")
    ...
    def test_deferred_context_resolution(self):
        ...
        total_attempts = len(resolution_results['successful_resolutions']) + len(resolution_results['failed_resolutions'])
...
class TestWildcardAndMissingFieldHandling:
    ...
    def test_optional_field_handling(self):
        ...
        try:
            result = structure._resolve_in_current_node_context("optional_list", "task.optional_fields")
    ...
    def test_nested_missing_field_handling(self):
        ...
        with pytest.raises((FieldValidationError, KeyError, AttributeError)) as exc_info:
            structure.add(TaskNestedMissing)

        # Error message should be informative and mention the missing field
        error_msg = str(exc_info.value).lower()
        assert ("missing_subfield" in error_msg or
                "does not exist" in error_msg or
                "not found" in error_msg), f"Error should be informative about missing field: {exc_info.value}"
...
class TestWildcardImplementationNeeds:
    ...
    def test_wildcard_vs_specific_field_priority_5a(self):
        ...
        variables = structure._variable_registry.variables
        data_vars = [v for v in variables.values() if v.variable_path == "data" and v.get_scope_name() == "prompt"]
...
class TestComprehensiveValidation:
    ...
    def test_circular_dependency_detection(self):
        ...
        for node_name, node in structure._root_nodes.items():
            ...
            if hasattr(node, 'extracted_commands'):
                ...
                for i, command in enumerate(node.extracted_commands):
                    print(f"      {i}: dest={command.destination_path}, type={command.command_type}")
            else:
        ...
        print(f"Circular dependencies found: {validation_result['circular_dependencies']}")
    ...
    def test_unsatisfied_variable_validation(self):
        ...
        assert not validation_result['is_valid']
        assert 'unsatisfied_variables' in validation_result or 'pending_targets' in validation_result
    ...
    def test_malformed_command_validation(self):
        ...
        try:
            ...
            assert ("malformed" in str(e).lower() or "syntax" in str(e).lower() or
                    "unclosed" in str(e).lower() or "brackets" in str(e).lower() or
                    "braces" in str(e).lower() or "parentheses" in str(e).lower())
    ...
    def test_impossible_variable_mapping_validation(self):
        ...
        assert ("@each requires at least one iterable" in str(exc_info.value) or
                "cannot be defined on non-iterable field" in str(exc_info.value))
...
class TestLangTreeIntegrationLayerBasics:
    ...
    def test_integration_module_imports(self):
        ...
        from langtree.prompt.integration import (
            LangTreeChainBuilder, PromptAssembler, ContextPropagator, ExecutionOrchestrator
        )
    ...
    def test_component_interfaces_exist(self):
        ...
        from langtree.prompt.integration import (
            LangTreeChainBuilder, ExecutionOrchestrator
        )
...
class TestLangTreeChainBuilderAdversarial:
    ...
    def test_topological_plan_with_circular_dependencies(self):
        ...
        try:
            ...
            assert a_node and b_node and c_node, f"All circular nodes should be present: A={a_node}, B={b_node}, C={c_node}"
            
            # Check for circular relationships
            has_circular_structure = False
            if (c_node in dependencies.get(a_node, set()) and 
                a_node in dependencies.get(b_node, set()) and 
                b_node in dependencies.get(c_node, set())):
            ...
            if has_circular_structure:
                ...
                assert len(circular_chains) > 0, f"Circular dependency detection should find cycles in: {dependencies}"
                ...
                assert cycle_found, f"Detected circular chains should include our A-B-C cycle: {circular_chains}"
            
        except ValueError as e:
            # Expected behavior: Should raise ValueError about circular dependencies
            error_msg = str(e).lower()
            assert any(term in error_msg for term in ['circular', 'cycle', 'dependency']), \
                f"Error should mention circular dependencies: {e}"
            
        except RuntimeError as e:
            # Also acceptable: Runtime error during topological sort
            error_msg = str(e).lower()
            assert any(term in error_msg for term in ['circular', 'cycle', 'dependency', 'topological']), \
        ...
        try:
            ...
            assert chain is not None, "Chain should either fail or succeed with resolution strategy"
            
            # Test that the chain can still be introspected
            graph = chain.get_graph()
            assert graph is not None, "Chain graph should be accessible even with resolved cycles"
            
        except (ValueError, RuntimeError) as e:
            # Expected: Chain building should fail with circular dependency error
            error_msg = str(e).lower()
            expected_terms = ['circular', 'cycle', 'dependency', 'validation', 'topological']
...
class TestLangTreeChainIntrospection:
    ...
    def test_simple_chain_structure_validation(self, mock_llm_provider):
        ...
        assert source_idx is not None, f"TaskSource not found in step order: {step_order}"
        assert processor_idx is not None, f"TaskProcessor not found in step order: {step_order}"
        assert sink_idx is not None, f"TaskSink not found in step order: {step_order}"
        
        assert source_idx < processor_idx, f"Source ({source_idx}) should come before Processor ({processor_idx})"
        assert processor_idx < sink_idx, f"Processor ({processor_idx}) should come before Sink ({sink_idx})"
        ...
        assert len(flows) >= 2, f"Should have at least 2 variable flows (source->processor, processor->sink), got {len(flows)}: {flows}"
        
        # Verify each flow has required structure
        for i, flow in enumerate(flows):
            # Check actual structure from implementation - may use 'from' instead of 'from_node'
            assert 'from' in flow or 'from_node' in flow, f"Flow {i} missing source field ('from' or 'from_node'): {flow}"
            assert 'target_node' in flow or 'to' in flow, f"Flow {i} missing target field ('target_node' or 'to'): {flow}"
            assert 'relationship_type' in flow, f"Flow {i} missing 'relationship_type': {flow}"
            ...
            assert from_node is not None and len(str(from_node)) > 0, f"Flow {i} has empty source: {flow}"
            assert target_node is not None and len(str(target_node)) > 0, f"Flow {i} has empty target: {flow}"
        ...
        source_related_flows = any(
            ('source' in str(from_field).lower() or 'source' in str(target_field).lower())
            for from_field, target_field in flow_pairs
        )
        assert source_related_flows, f"Should have source-related flows. Flows: {flow_pairs}"
        
        # Must have flow that involves processor and sink-related targets  
        processor_related_flows = any(
            ('processor' in str(from_field).lower() or 'processor' in str(target_field).lower())
            for from_field, target_field in flow_pairs
        )
        assert processor_related_flows, f"Should have processor-related flows. Flows: {flow_pairs}"
        
        # Verify relationship types are correct
        relationship_types = [flow['relationship_type'] for flow in flows]
        assert 'dependency' in relationship_types, f"Should have dependency relationships, got: {relationship_types}"
        ...
        assert not invalid_scopes, f"Invalid scopes found: {invalid_scopes}. Valid scopes: {valid_scopes}"
    ...
    def test_complex_chain_structure_validation(self, mock_llm_provider):
        ...
        source_idx = next(i for i, tag in enumerate(step_order) if 'source' in tag.lower())
        processor_a_idx = next(i for i, tag in enumerate(step_order) if 'processor_a' in tag.lower())
        processor_b_idx = next(i for i, tag in enumerate(step_order) if 'processor_b' in tag.lower())
        aggregator_idx = next(i for i, tag in enumerate(step_order) if 'aggregator' in tag.lower())
        ...
        assert processor_a_idx < aggregator_idx, "ProcessorA should come before Aggregator"
        assert processor_b_idx < aggregator_idx, "ProcessorB should come before Aggregator"
        
        # Verify variable flows capture the branching structure
        flows = execution_plan['variable_flows']
        flow_pairs = [(flow.get('from_node', ''), flow.get('target_node', '')) for flow in flows]
        
        # Should have flows from source to both processors
        source_to_a = any('source' in from_node.lower() and 'processor_a' in to_node.lower() 
                         for from_node, to_node in flow_pairs)
        source_to_b = any('source' in from_node.lower() and 'processor_b' in to_node.lower() 
                         for from_node, to_node in flow_pairs)
        
        # Should have flows from both processors to aggregator
        a_to_aggregator = any('processor_a' in from_node.lower() and 'aggregator' in to_node.lower() 
                             for from_node, to_node in flow_pairs)
        b_to_aggregator = any('processor_b' in from_node.lower() and 'aggregator' in to_node.lower() 
                             for from_node, to_node in flow_pairs)
    ...
    def test_large_tree_execution_plan_properties(self):
        ...
        for i, letter in enumerate(['A', 'B', 'C']):
            for j in [1, 2]:
                ...
                level2_class = type(f'TaskLevel2{letter}{j}', (PromptTreeNode,), class_dict)
            ...
            for j in [1, 2]:
                for k in ['A', 'B']:
                    class_dict = {
                        ...
                        '__annotations__': {f'level3_{letter.lower()}{j}{k.lower()}_data': str},
                        f'level3_{letter.lower()}{j}{k.lower()}_data': f"final_result_{letter}{j}{k}"
                    }
                    level3_class = type(f'TaskLevel3{letter}{j}{k}', (PromptTreeNode,), class_dict)
        ...
        assert len(execution_plan['chain_steps']) == total_nodes, f"Should have {total_nodes} chain steps"
        ...
        level1_indices = [i for i, tag in enumerate(step_order) if 'level1' in tag.lower()]
        level2_indices = [i for i, tag in enumerate(step_order) if 'level2' in tag.lower()]
        level3_indices = [i for i, tag in enumerate(step_order) if 'level3' in tag.lower()]
        ...
        assert max(level1_indices) < min(level2_indices), "All Level1 should come before Level2"
        assert max(level2_indices) < min(level3_indices), "All Level2 should come before Level3"
        
        # Verify variable flows match the tree structure
        flows = execution_plan['variable_flows']
        assert len(flows) >= 21, "Should have flows for all dependencies (3+6+12)"  # 3 from root, 6 from level1, 12 from level2
    ...
    def test_chain_execution_simulation(self, mock_llm_provider):
        ...
        collector_step = next(step for step in execution_plan['chain_steps'] 
                             if 'collector' in step['node_tag'].lower())
        transformer_step = next(step for step in execution_plan['chain_steps'] 
                               if 'transformer' in step['node_tag'].lower())
        outputter_step = next(step for step in execution_plan['chain_steps'] 
        ...
        collector_to_transformer = any(
            'collector' in flow.get('from_node', '').lower() and 
            'transformer' in flow.get('target_node', '').lower()
            for flow in flows
        )
        
        # Should have flow from transformer to outputter (transformed)
        transformer_to_outputter = any(
            'transformer' in flow.get('from_node', '').lower() and 
            'outputter' in flow.get('target_node', '').lower()
            for flow in flows
        )
        
        assert collector_to_transformer, "Should have data flow from Collector to Transformer"
        assert transformer_to_outputter, "Should have data flow from Transformer to Outputter"
    ...
    def test_chain_step_content_validation(self):
        ...
        assert len(chain_steps) >= 2, f"Expected at least 2 chain steps, got {len(chain_steps)}: {step_tags}"
        ...
        assert processor_step is not None, f"Document processor step not found in execution plan: {step_tags}"

        # Test prompt assembly for document processor step
        processor_node = run_structure.get_node(processor_step['node_tag'])
        assert processor_node is not None, f"Document processor node not found for tag: {processor_step['node_tag']}"
        ...
        for section in required_sections:
            assert section in assembled_prompt, f"Assembled prompt missing '{section}' section"
        
        # Verify system prompt content and template variable resolution
        system_prompt = assembled_prompt['system']
        assert system_prompt is not None and len(system_prompt) > 0, "System prompt should not be empty"
        
        # Template variables should be resolved (not present in raw form)
        assert '{PROMPT_SUBTREE}' not in system_prompt, f"Template variable {{PROMPT_SUBTREE}} should be resolved. Content: {system_prompt[:200]}..."
        assert '{COLLECTED_CONTEXT}' not in system_prompt, f"Template variable {{COLLECTED_CONTEXT}} should be resolved. Content: {system_prompt[:200]}..."
        
        # Should contain analyzer-specific content
        assert 'analyzer' in system_prompt.lower(), f"Should contain analyzer content. Content: {system_prompt[:200]}..."

        # Should contain original docstring content
        assert 'processes the rich prompt data' in system_prompt.lower(), f"Should contain original docstring content. Content: {system_prompt[:200]}..."
        
        # Verify task section extraction
        task_section = assembled_prompt['task']
        assert task_section is not None and len(task_section) > 0, "Task section should not be empty"
        
        # Verify output section contains field descriptions
        output_section = assembled_prompt['output']
        assert output_section is not None and len(output_section) > 0, "Output section should not be empty"
        assert 'source_data' in output_section, "Output section should describe source_data field"
        assert 'processed_analysis' in output_section, "Output section should describe processed_analysis field"
        ...
        try:
            ...
            assert len(mermaid) > 50, "Chain graph should generate substantial mermaid diagram"

        except Exception as e:
            # Chain building might fail due to missing LLM setup, but error should be meaningful
            error_str = str(e).lower()
            expected_errors = ["model", "not found", "not defined", "provider", "api", "key", "llm"]
            assert any(err in error_str for err in expected_errors), f"Chain building should fail with expected LLM error, got: {e}"

        # AGGRESSIVE TEST: Verify context propagation wrapper functionality
        context_propagator = builder.context_propagator
        assert hasattr(context_propagator, 'wrap_with_context_propagation'), "Context propagator should have wrapping method"
        ...
        wrapped_chain = context_propagator.wrap_with_context_propagation(dummy_chain, processor_step['node_tag'], processor_node)

        assert wrapped_chain is not None, "Context propagation wrapper should be created"
        ...
        try:
            ...
            # Chain building might fail due to missing LLM setup, but error should be meaningful
            error_str = str(e).lower()
            expected_errors = ["model", "not found", "not defined", "provider", "api", "key"]
            assert any(err in error_str for err in expected_errors), f"Chain building should fail with expected LLM error, got: {e}"
    ...
    def test_aggressive_chain_structure_introspection(self):
        """AGGRESSIVE TEST: Deep introspection of actual chain structure and components."""
        from langtree.prompt.integration import LangTreeChainBuilder
        ...
        class TaskLinearMiddle(PromptTreeNode):
            ...
            """
            linear_processed: str = Field(
                default="middle_value",
        ...
        class TaskLinearEnd(PromptTreeNode):
            ...
            """
            linear_result: str = "end_value"
        
        run_structure.add(TaskLinearStart)
        run_structure.add(TaskLinearMiddle)
        run_structure.add(TaskLinearEnd)
        
        builder = LangTreeChainBuilder(run_structure)
        
        # Act: Build chain and perform DEEP INTROSPECTION
        try:
            chain = builder.build_execution_chain()
            execution_plan = builder._get_topological_execution_plan()
            
            # AGGRESSIVE TEST 1: Chain object introspection
            assert chain is not None, "Chain must not be None"
            
            # Test chain type and basic properties
            chain_type = type(chain).__name__
            print(f"DEBUG: Built chain type: {chain_type}")
            
            # AGGRESSIVE TEST 2: Inspect chain internal structure
            # Check if chain has steps (RunnableSequence pattern)
            if hasattr(chain, 'steps'):
                steps = chain.steps
                print(f"DEBUG: Chain has {len(steps)} steps")
                assert len(steps) >= 3, f"Linear chain should have at least 3 steps, got {len(steps)}"
                
                # Inspect each step
                for i, step in enumerate(steps):
                    step_type = type(step).__name__
                    print(f"DEBUG: Step {i}: {step_type}")
                    
                    # Each step should be a runnable
                    assert hasattr(step, 'invoke'), f"Step {i} ({step_type}) should be invokable"
            
            # AGGRESSIVE TEST 3: Graph introspection (if available)
            try:
                graph = chain.get_graph()
                ...
                for node_id, node_data in graph.nodes.items():
                    node_label = node_data.get('data', {}).get('label', str(node_id))
                    node_labels.append(node_label)
                    print(f"DEBUG: Graph node {node_id}: {node_label}")
                
                # Should have nodes corresponding to our tasks
                assert len(graph.nodes) >= 3, f"Graph should have at least 3 nodes for linear chain, got {len(graph.nodes)}"
                
                # Verify graph connectivity
                assert len(graph.edges) >= 2, f"Graph should have at least 2 edges for linear chain, got {len(graph.edges)}"
                
                # Test graph visualization
                try:
                    mermaid = graph.draw_mermaid()
                    assert isinstance(mermaid, str) and len(mermaid) > 100, "Should generate substantial mermaid diagram"
                    print(f"DEBUG: Generated mermaid diagram ({len(mermaid)} chars)")
                    
                    # Mermaid should contain references to our tasks
                    task_refs_in_mermaid = sum(1 for task in ['start', 'middle', 'end'] 
                                              if task in mermaid.lower())
                    assert task_refs_in_mermaid >= 2, f"Mermaid should reference our tasks, found {task_refs_in_mermaid} references"
                    
                except Exception as graph_error:
                    print(f"DEBUG: Graph visualization failed (acceptable): {graph_error}")
                    
            except Exception as graph_error:
                print(f"DEBUG: Graph introspection failed (acceptable): {graph_error}")
            
            # AGGRESSIVE TEST 4: Execution plan structure validation
            assert 'chain_steps' in execution_plan, "Execution plan must have chain_steps"
            
            steps = execution_plan['chain_steps']
            assert len(steps) == 3, f"Should have exactly 3 steps for linear chain, got {len(steps)}"
            
            # Verify step structure and content
            step_tags = [step['node_tag'] for step in steps]
            print(f"DEBUG: Step tags: {step_tags}")
            
            # Check that steps reference our actual nodes
            expected_fragments = ['linearstart', 'linearmiddle', 'linearend']
            for fragment in expected_fragments:
                matching_steps = [tag for tag in step_tags if fragment in tag.lower()]
                assert len(matching_steps) >= 1, f"Should have step matching '{fragment}', got tags: {step_tags}"
            
            # AGGRESSIVE TEST 5: Variable flow introspection
            assert 'variable_flows' in execution_plan, "Execution plan must have variable_flows"
            
            flows = execution_plan['variable_flows']
            assert len(flows) >= 2, f"Linear chain should have at least 2 variable flows, got {len(flows)}"
            
            # Inspect flow structure
            for i, flow in enumerate(flows):
                print(f"DEBUG: Flow {i}: {flow}")
                
                # Validate flow structure
                required_fields = ['relationship_type', 'scope']
                for field in required_fields:
                    assert field in flow, f"Flow {i} missing required field '{field}': {flow}"
                
                # Check flow references actual nodes/fields
                from_ref = flow.get('from_node', flow.get('from', ''))
                to_ref = flow.get('target_node', flow.get('to', ''))
                
                assert from_ref and len(str(from_ref)) > 0, f"Flow {i} has empty source reference: {flow}"
                assert to_ref and len(str(to_ref)) > 0, f"Flow {i} has empty target reference: {flow}"
            
            # AGGRESSIVE TEST 6: Prompt content introspection
            if 'prompt_content' in execution_plan:
                prompt_content = execution_plan['prompt_content']
                
                for step_tag, content in prompt_content.items():
                    print(f"DEBUG: Prompt for {step_tag}: {content[:100]}...")
                    
                    # Verify prompt contains task-relevant content
                    assert len(content) > 50, f"Prompt for {step_tag} should be substantial, got {len(content)} chars"
                    
                    # Check for template variable processing
                    remaining_templates = [var for var in ['{PROMPT_SUBTREE}', '{COLLECTED_CONTEXT}'] 
                                         if var in content]
                    if remaining_templates:
                        print(f"DEBUG: Unprocessed templates in {step_tag}: {remaining_templates}")
            
            # AGGRESSIVE TEST 7: Test actual chain composition patterns
            # Try to identify common LangChain patterns
            if hasattr(chain, 'first') and hasattr(chain, 'last'):
                print(f"DEBUG: Chain appears to be a sequence with first={type(chain.first).__name__}, last={type(chain.last).__name__}")
            
            if hasattr(chain, 'steps__'):
                step_dict = chain.steps__
                print(f"DEBUG: Chain step dictionary: {list(step_dict.keys())}")
                
                # Verify step naming matches our tasks
                step_names = list(step_dict.keys())
                task_matches = sum(1 for name in step_names 
                                  for task in ['start', 'middle', 'end'] 
                                  if task in name.lower())
                assert task_matches >= 2, f"Step names should reflect our tasks, got: {step_names}"
            
            print("DEBUG: Chain introspection completed successfully!")
            
        except Exception as e:
            # If chain building fails, verify it fails for expected reasons
            error_msg = str(e).lower()
            expected_errors = ['validation', 'model', 'provider', 'not found', 'not defined']
            
            if any(expected in error_msg for expected in expected_errors):
                print(f"DEBUG: Chain building failed with expected error: {e}")
                pytest.skip(f"Chain building failed with expected validation/setup error: {e}")
            else:
                pytest.fail(f"Chain building failed with unexpected error: {e}")
    ...
    def test_chain_component_naming_and_identification(self):
        ...
        parallel_chain = RunnableParallel({
            "source": source_runnable,
            "processor": processor_runnable
        })
        ...
        assert "TaskSource" in parallel_mermaid, "Should identify TaskSource in parallel"
        assert "TaskProcessor" in parallel_mermaid, "Should identify TaskProcessor in parallel"
        
        # Test sequence introspection  
        sequence_graph = sequence_chain.get_graph()
        sequence_mermaid = sequence_graph.draw_mermaid()
        
        assert "TaskSource" in sequence_mermaid, "Should identify TaskSource in sequence"
        assert "TaskProcessor" in sequence_mermaid, "Should identify TaskProcessor in sequence"
        ...
        assert source_pos < sink_pos, "TaskSource should appear before TaskSink in sequence"
        ...
        if hasattr(parallel_chain, 'steps__'):
            ...
            assert steps['source'].name == "TaskSource", "Source should have correct name"
            assert steps['processor'].name == "TaskProcessor", "Processor should have correct name"
...
class TestLangTreeChainBuilderAdversarialContinued:
    ...
    def test_build_step_chain_with_missing_node(self):
        ...
        execution_plan = builder._get_topological_execution_plan()
        valid_steps = [step for step in execution_plan['chain_steps'] if 'valid' in step['node_tag'].lower()]
        ...
        try:
            ...
            error_msg = str(e).lower()
            validation_terms = ['validation', 'unresolved', 'missing', 'target', 'failed']
    ...
    def test_build_execution_chain_with_invalid_structure(self):
        ...
        try:
            ...
            assert total_errors > 0, f"Should report validation errors, got: {validation_result}"
            
            # Verify specific error types are caught
            error_types = []
            if 'self_references' in validation_result and validation_result['self_references']:
                error_types.append('self_references')
            if 'circular_dependencies' in validation_result and validation_result['circular_dependencies']:
                error_types.append('circular_dependencies')
            if 'unsatisfied_variables' in validation_result and validation_result['unsatisfied_variables']:
                error_types.append('unsatisfied_variables')
            if 'invalid_scope_references' in validation_result and validation_result['invalid_scope_references']:
                error_types.append('invalid_scope_references')
            
            assert len(error_types) > 0, f"Should detect specific error types, got validation result: {validation_result}"
            
        except Exception as e:
            # If validation method doesn't exist, that's also informative
            error_str = str(e).lower()
            method_missing = any(term in error_str for term in ['attribute', 'method', 'not found'])
            if method_missing:
        ...
        try:
            ...
            execution_plan = builder._get_topological_execution_plan()
            assert 'chain_steps' in execution_plan, "Execution plan should have structure"
            ...
            self_ref_steps = [tag for tag in step_tags if 'self_reference' in tag.lower()]
            if self_ref_steps:
                pytest.fail("Self-referencing nodes should not be included in execution plan")
            
        except (ValueError, RuntimeError) as e:
            # Expected: Should fail with validation or structural error
            error_msg = str(e).lower()
            expected_error_terms = [
                'validation', 'circular', 'self', 'reference', 'invalid', 
                'unresolved', 'missing', 'scope', 'dependency'
            ]
        ...
        try:
            ...
            assert 'chain_steps' in execution_plan, "Should have chain_steps even if empty"
                ...
                for i, node_a in enumerate(dep_order):
                    for j, node_b in enumerate(dep_order):
                        if i < j:  # node_a comes before node_b
                            ...
                            assert node_a != node_b, f"Duplicate nodes in dependency order: {dep_order}"
    ...
    def test_compose_dependency_chain_with_impossible_dependencies(self):
        ...
        execution_plan_self = {
            'chain_steps': [
                {'node_tag': 'step_self', 'dependencies': ['step_self']}
            ]
        }
        
        # Scenario 4: Missing dependency (A -> NonExistent)
        chain_steps_missing: Dict[str, MockRunnable] = {
            'step_a': MockRunnable("StepA")
        }
        ...
        for scenario_name, chain_steps, execution_plan in scenarios:
            ...
            if scenario_name == "circular" or scenario_name == "indirect_circular":
                assert any(term in error_msg for term in ['circular', 'cycle', 'dependency loop']), \
                    f"Circular dependency should mention cycles: {error_msg}"
            elif scenario_name == "self_dependency":
                assert any(term in error_msg for term in ['self', 'circular', 'cycle']), \
                    f"Self-dependency should mention self-reference: {error_msg}"
            elif scenario_name == "missing_dependency":
                assert any(term in error_msg for term in ['missing', 'not found', 'nonexistent']), \
        ...
        complex_execution_plan = {
            'chain_steps': [
                {'node_tag': 'step_1', 'dependencies': ['step_2', 'nonexistent']},  # Missing dep
        ...
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            builder._compose_dependency_chain(complex_chain_steps, complex_execution_plan)
        
        # Should detect and report at least one of the issues
        error_msg = str(exc_info.value).lower()
        cycle_detected = any(term in error_msg for term in ['circular', 'cycle'])
        missing_detected = any(term in error_msg for term in ['missing', 'not found', 'nonexistent'])
        ...
        malformed_scenarios = [
            ...
            ("malformed_dependencies", {'step_a': MockRunnable("A")}, {
        ...
        for scenario_name, steps, plan in malformed_scenarios:
            try:
                ...
                assert result is not None, f"Should return something for scenario {scenario_name}"
                
            except (ValueError, RuntimeError, TypeError, KeyError) as e:
                # Expected - should fail with structural error
                error_msg = str(e).lower()
                expected_terms = ['invalid', 'malformed', 'missing', 'format', 'structure']
                assert any(term in error_msg for term in expected_terms), \
        ...
        assert any(keyword in str(exc_info.value).lower() for keyword in
                   ["circular", "impossible", "unsatisfiable", "deadlock"])
    ...
    def test_build_step_chain_with_malformed_syntax(self):
        ...
        malformed_syntax_cases = [
            ...
            ("TripleBraces", "! @->task.target@{{{value.data=field}}}"),  # Triple braces
            ("MissingEquals", "! @->task.target@{{value.data:field}}"),   # Colon instead of equals
            ("EmptyAssignment", "! @->task.target@{{value.data=}}"),      # Empty value
            
            # Invalid target paths
            ("EmptyTarget", "! @->@{{value.data=field}}"),                # Empty target
            ("InvalidTargetChars", "! @->task.target$%^@{{value.data=field}}"),  # Special chars
            ("SpacesInTarget", "! @->task. target@{{value.data=field}}"), # Spaces in target
        ...
        for test_name, malformed_command in malformed_syntax_cases:
            ...
            node_class = type(f'Task{test_name}', (PromptTreeNode,), {
                '__doc__': f"""
                {malformed_command}
            ...
            try:
                ...
                try:
                    builder.build_execution_chain()
                    pytest.fail(f"Malformed syntax {test_name} should have been rejected: {malformed_command}")
                    
                except (ValueError, RuntimeError, Exception) as e:
                    # Expected - should fail with syntax/validation error
                    error_msg = str(e).lower()
                    syntax_terms = ['syntax', 'malformed', 'invalid', 'parse', 'command', 'format']
        ...
        boundary_cases = [
            ...
            ("VeryLongNames", f"! @->task.{'x' * 100}@{{{{value.{'y' * 50}={'z' * 30}}}}}"),
            
            # Minimal valid syntax
            ("MinimalValid", "! @->task.a@{{value.b=c}}"),
            
            # Multiple valid assignments
            ("MultipleValid", "! @->task.target@{{value.a=field1}} @->task.other@{{value.b=field2}}"),
        ...
        for test_name, boundary_command in boundary_cases:
            ...
            node_class = type(f'TaskBoundary{test_name}', (PromptTreeNode,), {
                '__doc__': f"""
                {boundary_command}
                ...
                'field_456': 'data456',
                '__annotations__': {
                    'field_data': str, 'field1': str, 'field2': str, 'field_456': str
                }
            })
            ...
            try:
                ...
                if any(term in error_msg for term in syntax_terms):
                    pytest.fail(f"Boundary case {test_name} failed with syntax error (should be valid): {e}")
                elif any(term in error_msg for term in semantic_terms):
                    # Acceptable - semantic validation can reject boundary cases
                    pass
                else:
                    pytest.fail(f"Boundary case {test_name} failed with unexpected error: {e}")
        ...
        try:
            ...
            if 'syntax' in error_msg or 'malformed' in error_msg:
                pytest.fail(f"Complex valid syntax should not fail with syntax error: {e}")
    ...
    def test_topological_plan_with_massive_dependency_graph(self):
        ...
        for i in range(50):
            ...
            node_class = type(f'TaskLinearNode{i}', (PromptTreeNode,), {
                '__doc__': docstring,
                **field_attrs
            })
            run_structure.add(node_class)
        ...
        for i in range(20):
            ...
            """
            node_class = type(f'TaskFanMiddle{i}', (PromptTreeNode,), {
                '__doc__': docstring,
        ...
        for diamond in range(5):
            # Diamond root
            diamond_root_class = type(f'TaskDiamondRoot{diamond}', (PromptTreeNode,), {
            ...
            for branch in range(2):
                branch_class = type(f'TaskDiamondBranch{diamond}Branch{branch}', (PromptTreeNode,), {
            ...
            merger_class = type(f'TaskDiamondMerger{diamond}', (PromptTreeNode,), {
                ...
                '__annotations__': {'merged_result': str, 'branch0': str, 'branch1': str}
            })
        ...
        try:
            ...
            assert planning_time < 30.0, f"Topological planning took {planning_time:.2f}s, too slow for large graph"
            ...
            expected_min = 50 + 1 + 20 + 1 + (5 * 4)  # linear + fan + diamonds = ~96 minimum
            ...
            if 'complex' in error_msg or 'large' in error_msg or 'limit' in error_msg:
                ...
                pytest.fail(f"Planning failed after {planning_time:.2f}s with error: {e}")
        ...
        try:
            ...
            try:
                ...
                assert cycle_time < 10.0, f"Cycle detection took {cycle_time:.2f}s, too slow"
        ...
        for iteration in range(5):
            ...
            try:
                ...
                assert stress_time < 30.0, f"Iteration {iteration} took {stress_time:.2f}s"
    ...
    def test_build_execution_chain_with_complex_variable_scopes(self):
        ...
        for test_name, invalid_command in invalid_scope_cases:
            node_class = type(f'TaskInvalidScope{test_name}', (PromptTreeNode,), {
                '__doc__': f"""
                {invalid_command}
        ...
        for test_name, invalid_assignment in assignment_edge_cases:
            node_class = type(f'TaskAssignment{test_name}', (PromptTreeNode,), {
                '__doc__': f"""
                {invalid_assignment}
            ...
            try:
                ...
                try:
                    builder.build_execution_chain()
                    pytest.fail(f"Invalid assignment {test_name} should have been rejected")
                    
                except (ValueError, RuntimeError) as e:
                    error_msg = str(e).lower()
                    assignment_terms = ['field', 'invalid', 'assignment', 'syntax', 'empty']
                ...
                # Expected - may fail at addition time with various error types
                error_msg = str(e).lower()
                assignment_terms = ['field', 'invalid', 'parse', 'syntax', 'path', 'empty']
                assert any(term in error_msg for term in assignment_terms), \
                    f"Should fail with assignment error for {test_name}: {e}"
        ...
        try:
            execution_chain = builder.build_execution_chain()
            assert execution_chain is not None, "Valid complex scopes should build successfully"
            ...
            step_count = len(execution_plan['chain_steps'])
            assert step_count >= 3, f"Should have steps for valid nodes, got {step_count}"
            
        except Exception as e:
            # Valid syntax should not fail
            if 'syntax' in str(e).lower() or 'malformed' in str(e).lower():
                pytest.fail(f"Valid complex scopes should not fail with syntax error: {e}")
    ...
    def test_build_execution_chain_with_template_variable_edge_cases(self):
        ...
        for test_name, invalid_template in invalid_template_cases:
            node_class = type(f'TaskInvalidTemplate{test_name}', (PromptTreeNode,), {
                '__doc__': f"""
            ...
            try:
                ...
                try:
                    ...
                    assert execution_chain is not None, "Should handle template processing"
                ...
                error_msg = str(e).lower()
                template_terms = ['template', 'variable', 'invalid', 'syntax', 'unknown']
        ...
        class TaskComplexCommands(PromptTreeNode):
            ...
            field1: str = Field(
                default="data1",
                description="! @->task.target1@{{value.data=field1}}"
            )
            field2: str = Field(
                default="data2",
        ...
        for test_name, edge_command in command_edge_cases:
            ...
            try:
                ...
                try:
                    ...
                    assert 'chain_steps' in execution_plan, "Should have valid execution plan"
                    ...
                    if any(term in error_msg for term in parse_terms):
                        ...
                        # Unexpected error type
                        pytest.fail(f"Unexpected error for command edge case {test_name}: {e}")
                        
            except Exception as e:
                # May fail at addition time with various error types
                error_msg = str(e).lower()
                parse_terms = ['parse', 'syntax', 'invalid', 'command', 'whitespace', 'space']
        ...
        try:
            ...
            error_msg = str(e).lower()
            if 'syntax' in error_msg or 'malformed' in error_msg or 'parse' in error_msg:
                pytest.fail(f"Valid complex syntax should not fail with parse error: {e}")
...
class TestAssemblyValidation:
    ...
    def test_assembly_validation_source_field_passes(self):
        ...
        class TaskSourceField(PromptTreeNode):
            ...
            existing_field: list[FieldGroup] = []  # EXISTS → source field with 2 nesting levels (matches iteration)
        ...
        assert assembly_errors == [], f"Assembly validation should pass for source fields, got: {assembly_errors}"
    ...
    def test_assembly_validation_no_iteration_passes(self):
        ...
        assembly_errors = builder._validate_assembly_phase()
        assert assembly_errors == [], f"Assembly validation should pass with no iteration, got: {assembly_errors}"
    ...
    def test_assembly_validation_mixed_mappings(self):
        ...
        error_msg = str(exc_info.value).lower()
        assert "requires at least one variable mapping to match iteration level" in error_msg
        assert "found nesting levels: [0]" in str(exc_info.value)
    ...
    def test_assembly_validation_timing_vs_semantic_validation(self):
        ...
        class TaskMixedFields(PromptTreeNode):
            ...
            source_field: str = ""  # EXISTS → source field with 0 levels → semantic validation fails
        ...
        assert len(assembly_errors) > 0, "Assembly validation should catch destination field nesting mismatch"
        ...
        try:
            ...
            assert "validation" in str(e).lower() or "nesting" in str(e).lower() or "level" in str(e).lower()