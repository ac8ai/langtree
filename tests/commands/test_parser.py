...
class TestCommandParser:
    ...
    def test_parse_valid_commands(self):
        ...
        for test_case in VALID_COMMANDS:
            try:
                ...
                assert isinstance(result, ParsedCommand), f"Failed to parse: {test_case.name}"
            except Exception as e:
                pytest.fail(f"Failed to parse valid command '{test_case.name}': {test_case.command}\nError: {e}")
    ...
    def test_wildcard_assignment(self):
        ...
        command = "! @->task.summarize_analysis@{{prompt.task_strategic_recommendations=*}}"
        result = self.parser.parse(command)
    ...
    def test_scope_modifier_parsing(self):
        ...
        test_cases = [
            ...
            ("task.field", "task"),
            ("unknown.field", "current_node"),  # Unknown scope modifier defaults to current_node
            ("p.field", "current_node"),  # Single character (unknown) defaults to current_node
        ...
        for target_path, expected_scope_name in test_cases:
            ...
            mapping = result.variable_mappings[0]
            assert mapping.resolved_target is not None
            assert mapping.resolved_target.scope is not None, f"Scope should never be None for path: {target_path}"
            actual_scope_name = mapping.resolved_target.scope.get_name()
            assert actual_scope_name == expected_scope_name, f"Failed for path '{target_path}'. Expected '{expected_scope_name}', got '{actual_scope_name}'"
...
class TestScopeResolution:
    ...
    def test_target_path_scope_extraction(self):
        ...
        test_cases = [
            ...
            ("! @->task.undefined@{{unknown.var=source}}", "current_node"),  # Unknown scope becomes current_node
            ("! @->task.undefined@{{var=source}}", "current_node"),  # No scope becomes current_node
        ]
        ...
        for command, expected_scope_name in test_cases:
            ...
            mapping = result.variable_mappings[0]
            assert mapping.resolved_target is not None
            assert mapping.resolved_target.scope is not None, f"Scope should never be None for command: {command}"
            actual_scope_name = mapping.resolved_target.scope.get_name()
            assert actual_scope_name == expected_scope_name, f"Failed for command: {command}. Expected '{expected_scope_name}', got '{actual_scope_name}'"
    ...
    def test_inclusion_path_scope_extraction(self):
        ...
        """
        test_cases = [
            ("! @each[prompt.sections]->task.undefined@{{value.var=prompt.sections.source}}*", "prompt", "sections"),
            ("! @each[value.items]->task.undefined@{{value.var=value.items.source}}*", "value", "items"),
            ("! @each[outputs.results]->task.undefined@{{value.var=outputs.results.source}}*", "outputs", "results"),
            ("! @each[task.data]->task.undefined@{{value.var=task.data.source}}*", "task", "data"),
            ("! @each[unknown.items]->task.undefined@{{value.var=unknown.items.source}}*", "current_node", "unknown.items"),
            ("! @each[items]->task.undefined@{{value.var=items.source}}*", "current_node", "items"),
        ]
        ...
        for command, expected_scope_name, expected_path in test_cases:
            ...
            assert result.resolved_inclusion.scope is not None, f"Scope should never be None for command: {command}"
            actual_scope_name = result.resolved_inclusion.scope.get_name()
            assert actual_scope_name == expected_scope_name, f"Failed scope for command: {command}. Expected '{expected_scope_name}', got '{actual_scope_name}'"
            assert result.resolved_inclusion.path == expected_path, f"Failed path for command: {command}. Expected '{expected_path}', got '{result.resolved_inclusion.path}'"
    ...
    def test_destination_path_scope_extraction(self):
        ...
        test_cases = [
            ...
            ("! @->unknown.target@{{value.var=source}}", "current_node", "unknown.target"),
        ...
        for command, expected_scope_name, expected_path in test_cases:
            ...
            assert result.resolved_destination.scope is not None, f"Scope should never be None for command: {command}"
            actual_scope_name = result.resolved_destination.scope.get_name()
            assert actual_scope_name == expected_scope_name, f"Failed scope for command: {command}. Expected '{expected_scope_name}', got '{actual_scope_name}'"
            assert result.resolved_destination.path == expected_path, f"Failed path for command: {command}. Expected '{expected_path}', got '{result.resolved_destination.path}'"
    ...
    def test_source_path_scope_extraction(self):
        ...
        test_cases = [
            ...
            ("! @->task@{{value.var=unknown.source}}", "current_node", "unknown.source"),
        ...
        for command, expected_scope_name, expected_path in test_cases:
            ...
            assert mapping.resolved_source.scope is not None, f"Scope should never be None for command: {command}"
            actual_scope_name = mapping.resolved_source.scope.get_name()
            assert actual_scope_name == expected_scope_name, f"Failed scope for command: {command}. Expected '{expected_scope_name}', got '{actual_scope_name}'"
            assert mapping.resolved_source.path == expected_path, f"Failed path for command: {command}. Expected '{expected_path}', got '{mapping.resolved_source.path}'"
    ...
    def test_multiple_scopes_in_same_command(self):
        ...
        command = "! @each[prompt.sections]->value.result_field@{{outputs.title=task.data}}*"
        result = parse_command(command)
    ...
    def test_same_scope_in_multiple_places(self):
        ...
        """
        command = "! @each[prompt.items]->prompt.destination@{{prompt.var=prompt.source}}*"
        result = parse_command(command)
    ...
    def test_confusing_edge_case_scope_as_field_name(self):
        ...
        command = "! @each[current.prompt]->task.processor@{{value.result=current.prompt}}*"
        result = parse_command(command)
...
class TestVariableAssignmentCommands:
    ...
    def test_invalid_unquoted_value(self):
        ...
        test_cases = [
            ("! var=invalid_string", "Unquoted value must be a number or boolean"),    # Unquoted non-numeric string
            ("! var=mixed123string", "Unquoted value must be a number or boolean"),    # Mixed alphanumeric
...
class TestExecutionCommands:
    ...
    def test_parse_command_no_arguments(self):
        ...
        with pytest.raises(CommandParseError, match="resample command requires exactly 1 argument"):
            self.parser.parse("! resample()")
        
        with pytest.raises(CommandParseError, match="llm command requires 1-2 arguments"):
    ...
    def test_unknown_command_error(self):
        ...
        for command in invalid_commands:
            with pytest.raises(CommandParseError, match="Unknown command.*Valid commands are: llm, resample"):
                self.parser.parse(command)
    ...
    def test_resample_argument_validation(self):
        ...
        with pytest.raises(CommandParseError, match="resample n_times must be positive"):
        ...
        with pytest.raises(CommandParseError, match="resample argument must be int or variable name"):
    ...
    def test_llm_argument_validation(self):
        ...
        with pytest.raises(CommandParseError, match="llm override must be bool or variable name"):
...
class TestResamplingCommands:
    ...
    def test_invalid_field_name_format(self):
        ...
        test_cases = [
            ("! @resampled[123invalid]->mean", "Invalid field name"),    # Starts with number
            ("! @resampled[invalid-name]->mean", "Invalid field name"),  # Contains hyphen
            ("! @resampled[invalid name]->mean", "Invalid field name"),  # Contains space
            ("! @resampled[]->mean", "Empty field name in resampling command"),  # Empty field name
        ]
    ...
    def test_invalid_aggregation_function(self):
        ...
        test_cases = [
            ("! @resampled[field]->invalid", "Invalid aggregation function"),         # Unknown function
            ("! @resampled[field]->count", "Invalid aggregation function"),           # Not in valid functions list
            ("! @resampled[field]->aggregate", "Invalid aggregation function"),       # Generic term not allowed
            ("! @resampled[field]->", "Empty aggregation function in resampling command"),  # Empty function
        ]
...
class TestCommentParsing:
    ...
    def test_comment_whitespace_handling(self):
        ...
        commands = [
            ...
            ("! var=42 #   Multiple spaces", "Multiple spaces"),  # Leading spaces in comment trimmed
            ("! var=42 # Trailing spaces   ", "Trailing spaces"),   # Trailing spaces in comment trimmed
        ]
        
        for command_str, expected_comment in commands:
            result = self.parser.parse(command_str)
            # Only check comment for new command types that support comments
            if isinstance(result, (VariableAssignmentCommand, ExecutionCommand, ResamplingCommand)):
...
class TestBooleanValueSupport:
    ...
    def test_boolean_case_sensitivity(self):
        ...
        for command in invalid_commands:
            with pytest.raises(CommandParseError, match="Unquoted value must be a number"):
                self.parser.parse(command)
...
class TestNewCommandIntegrationWithExisting:
    ...
    def test_parse_command_type_detection(self):
        ...
        commands = [
            ...
            ("! @each[items]->task@{{value.item=items}}*", "ParsedCommand"),  # Existing type
...
class TestEdgeCases:
    ...
    def test_special_characters_in_strings(self):
        ...
        special_chars = "!@#$%^&*(){}[]|\\:;<>?,./"  # Removed quotes that would break parsing
...
def test_comment_examples_from_specification():
    ...
    comment_examples = [
        ...
        ("! resample(3) # Resample for better accuracy", "Resample for better accuracy"),
...
class TestNamedParameters:
    ...
    def test_invalid_named_parameter_for_llm(self):
        ...
        for command in invalid_commands:
            with pytest.raises(CommandParseError, match="llm command only supports 'override' named argument"):
                self.parser.parse(command)
    ...
    def test_resample_rejects_named_parameters(self):
        ...
        for command in invalid_commands:
            with pytest.raises(CommandParseError, match="resample command does not support named arguments"):
                self.parser.parse(command)
...
class TestNodeModifierCommands:
    ...
    def test_node_modifier_with_comments(self):
        ...
        test_cases = [
            ('! @sequential # Process fields in order', NodeModifierType.SEQUENTIAL, "Process fields in order"),
            ('! @parallel # Process fields simultaneously', NodeModifierType.PARALLEL, "Process fields simultaneously"),
            ('! together # Shorthand for parallel', NodeModifierType.TOGETHER, "Shorthand for parallel"),
        ]
...
class TestBooleanLiteralParsing:
    ...
    def test_invalid_boolean_literals(self):
        ...
        for command in invalid_commands:
            with pytest.raises(CommandParseError, match="Unquoted value must be a number or boolean"):
                self.parser.parse(command)
...
class TestStrictWhitespaceValidation:
    ...
    def test_no_spaces_around_brackets(self):
        ...
        for command in invalid_commands:
            with pytest.raises(CommandParseError, match="Space.*(before '\\['|after '\\]').*not allowed"):
                self.parser.parse(command)
...
class TestParserAggressiveEdgeCases:
    ...
    def test_malformed_paths_comprehensive(self):
        ...
        for i, command in enumerate(malformed_commands):
            ...
            try:
                ...
                assert False, f"Command should have failed but parsed successfully: {command}"
    ...
    def test_boundary_condition_commands(self):
        ...
        boundary_commands = [
            ...
            "! @each[" + "a" * 100 + "]->task@{{value.item=items}}*",      # Very long inclusion path
            "! @each[items]->" + "task." * 50 + "target@{{value.item=items}}*",  # Very long destination
    ...
    def test_unicode_and_special_characters(self):
        ...
        for i, command in enumerate(unicode_commands):
            ...
            try:
                ...
                assert False, f"Command with unicode/special chars should have failed: {command}"
            except CommandParseError as e:
                print(f"  Expected failure: {e}")
                # Expected behavior
                pass
    ...
    def test_comment_edge_cases(self):
        ...
        for command in comment_commands:
            try:
                ...
                if "#" in command and ("[" not in command.split("#")[1] or "{{" not in command.split("#")[1]):
                    assert result is not None
            except CommandParseError:
...
class TestIterableDepthTracking:
    ...
    def test_mixed_spacing_same_depth(self):
        ...
        assert depthA == depthB == 2, f"Expected both depths to be 2, got A={depthA}, B={depthB}"
...
class TestLastMatchingIterableValidation:
    ...
    def test_complete_coverage_valid(self):
        ...
        result = self.parser.validate_last_matching_iterable(node, inclusion_path, rhs_paths)
    ...
    def test_non_iterable_only_paths_invalid(self):
        ...
        with pytest.raises(CommandParseError, match="@each requires at least one iterable"):
